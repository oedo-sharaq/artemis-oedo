...
using art::TSRPPACPlaneDqdxProcessor;

ClassImp(TSRPPACPlaneDqdxProcessor)

TSRPPACPlaneDqdxProcessor::TSRPPACPlaneDqdxProcessor()
{
//   Regi:vndssterInputInfo("Converter","name of dqdx converter",fConverterName,"srppac_xc0",
//                      &fConverter, "art::TMonotoneTableConverter");
   RegisterInputInfo("Converter","name of dqdx converter",fConverterName,"srppac_xc0",&fConverter,"art::TMonotoneTableConverter");
...
void TSRPPACPlaneDqdxProcessor::Process()
{
   double pos = 0.;
   double c0 = 0.;
   double c1 = 0.;
...
//      pos = (fTurned ? -1 : 1)  * ((id0 - fCenter + (1 - (*fConverter)->Convert(c0 - c1)) * ( id0 < id1 ? 1 : -1) * 0.5) * fStripWidth  - fDetOffset);
