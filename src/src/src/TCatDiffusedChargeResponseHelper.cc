 ...
 *         Last Modified : 2019-07-29 19:55:11 JST (ota)
...
void TCatDiffusedChargeResponseHelper::Copy(TCatDiffusedChargeResponseHelper &obj) const
{
   ...
   obj.fAttachment = fAttachment;
   obj.fDiffusionCoefficient = fDiffusionCoefficient;
   obj.fReadoutPlaneOffset = fReadoutPlaneOffset;
   obj.fWorkFunction = fWorkFunction;
   obj.fResponseTable = fResponseTable;
   obj.fRangeTable = fRangeTable;
   obj.fActiveAreaHalfX = fActiveAreaHalfX;
   obj.fActiveAreaHalfZ = fActiveAreaHalfZ;
   obj.fActiveAreaMargin = fActiveAreaMargin;
...
{
   proc->RegisterProcessorParameter("DefaultIntegrateStep","default integrate step (mm)",
                                    fDefaultIntegrateStep,1.);
   proc->RegisterProcessorParameter("Attachment","attachment coefficient (1/cm)",
                                    fAttachment,0.);
   proc->RegisterProcessorParameter("DiffusionCoefficient","diffusion coefficient (cm1/2)",
                                    fDiffusionCoefficient,0.04);
   proc->RegisterProcessorParameter("BaseDiffusion","base diffusion to be added to calculated diffusion (default 0)",
                                    fBaseDiffusion,0.);
   proc->RegisterProcessorParameter("ReadoutPlaneOffset","offset of readout plane relative to the beam axis",
                                    fReadoutPlaneOffset,-50.);
   proc->RegisterInputInfo("ReadoutPad","array of readout pad",
                           fNameReadoutPadArray,"readoutPad",
                           &fReadoutPadArray,TCatReadoutPadArray::Class_Name());
   proc->RegisterInputInfo("ResponseTableName","name of the response table",
                           fResponseTableName,"responseTable",
                           &fResponseTable,TNArray::Class_Name());
   proc->RegisterProcessorParameter("WorkFunction","value of work function (MeV)",
                                    fWorkFunction,37.*eV);
   proc->RegisterProcessorParameter("ActiveAreaHalfX","length in x direction of readout area to the edge from the center",
                                    fActiveAreaHalfX,50.);
   proc->RegisterProcessorParameter("ActiveAreaHalfZ","length in z direction of readout area to the edge from the center",
                                    fActiveAreaHalfZ,50.);

   proc->RegisterProcessorParameter("ActiveAreaMargin","active area margine to calculate the chage",
                                    fActiveAreaMargin,10.);
   proc->RegisterProcessorParameter("ActiveArea","active area as box (x1,y1,z1,x2,y2,z2)",fActiveArea,DoubleVec_t(6,0));

   proc->RegisterInputInfo("RangeTable","name of range table",fRangeTableName,TString("rangeTable"),
   ...
   proc->RegisterInputInfo("DisplacementTableZName","name of position displacement table for Z",fDisplacementTableZName,
                           TString(""));
   proc->RegisterProcessorParameter("UseDisplacement3D","use 3d-position to calculate displacement",fUseDisplacement3D,0);
...
//      printf("closest = %f, length = %f\n",closest, TMath::Sqrt((closestX - cx1) * (closestX - cx1) + (closestY - cy1) * ( closestY - cy1)));
   ...
   Double_t minDistance = TMath::Limits<Double_t>::Max();
   for (Int_t i = iStepStart; i < iStepEnd; ++i) {
      ...
      const Double_t diffusion_drift = sqrt(drift/10.) * fDiffusionCoefficient * 10.;
      const Double_t diffusion = TMath::Sqrt(fBaseDiffusion*fBaseDiffusion + diffusion_drift * diffusion_drift);
      ...
      const Double_t weight =(*fResponseTable)->Eval(x) * TMath::Exp(- fAttachment * drift/10.) ;
...
//          Double_t dedx = ((fR2DE)->Eval(fRange - i*fStep) +
//                                (fR2DE)->Eval(fRange - (i +1)*fStep)) * 0.5;
//         Double_t dedx = (*fRangeTable)->R2E(fRange - (i)*fStep)
//            - (*fRangeTable)->R2E(fRange - (i+1)*fStep);
         Double_t dedx = 0;
         if (fRange - (i) * fStep > 0) {
...
//         Double_t dedx = ((*fRangeTable)->R2DE(fRange - (i+0.5)*fStep)) * fStep;
...
void TCatDiffusedChargeResponseHelper::EstimateActiveRange()
{
   ...
   for (Int_t iDim = 0; iDim < kDimension; ++iDim) {
      if (startIsInside && !IsWithinRange(start[iDim],fActiveArea[iDim],fActiveArea[iDim+kDimension])) {
         startIsInside = kFALSE;
      }
      if (endIsInside && !IsWithinRange(end[iDim],fActiveArea[iDim],fActiveArea[iDim+kDimension])) {
         endIsInside = kFALSE;
      }
   }
   ...
   if (!endIsInside || !startIsInside) {
      ...
      for (Int_t iPlane = 0; iPlane < kPlanes; ++iPlane) {
         ...
         if ((fActiveArea[iPlane]-start[axis0])/unit[axis0] < 0) continue;
         TVector3 point = start + (fActiveArea[iPlane] - start[axis0])/unit[axis0] * unit;
         if ((point - start).Mag() > fRange) continue;
         if (IsWithinRange(point[axis1],fActiveArea[axis1],fActiveArea[axis1 + kDimension]) &&
             IsWithinRange(point[axis2],fActiveArea[axis2],fActiveArea[axis2 + kDimension])) {
...
Bool_t TCatDiffusedChargeResponseHelper::GeneratedElectronAt(TVector3 &point)
{
   ...
   Double_t y = point.Y();
   if (!IsWithinRange(y,fActiveArea[1],fActiveArea[4])) return kFALSE;
   Double_t drift = (y-fReadoutPlaneOffset);
   Double_t sigma = GetSigma(drift);
   Double_t x = point.X();
   Double_t z = point.Z();
   Double_t dx = 0.;
   Double_t dz = 0.;
