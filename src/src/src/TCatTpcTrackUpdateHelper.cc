 ...
 *         Last Modified : 2018-02-26 00:54:52 JST (ota)
...
#if 0
   parent->RegisterProcessorParameter("UpdateType","type of update for each axis, 0: no update, 1: fixed origin, 2: offset, 3: vertex",
                                      fUpdateType,IntVec_t(3,0));
   parent->RegisterProcessorParameter("Offset","Offset to be add to each axis for type 2 (offset) update",
                                      fOrigin,DoubleVec_t(3,0.));
   parent->RegisterInputCollection("Reference","reference for finding vertex",fNameReference,fNameReference);
#endif
}
