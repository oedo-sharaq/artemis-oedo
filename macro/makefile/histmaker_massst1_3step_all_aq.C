...
void histmaker_massst1_3step_all_aq(){
...
if(j==0){
 ...
 tree->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step.eventstore.170272.ca.root");
// tree->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step_unit.eventstore.170272.ca.root");
// TFile* file = TFile::Open("sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step.eventstore.170272.ca.root"); // Load file. Basically BLD file.

 TClonesArray *array_mass = NULL;
 TClonesArray *array_mass_corst1_BLD_3rd = NULL;
 art::TEventHeader *array_eventheader = NULL;
// TTree *tree = (TTree*) gDirectory->Get("tree");

 tree->SetBranchAddress("mass",&array_mass);
 tree->SetBranchAddress("mass_corst1_BLD_3rd",&array_mass_corst1_BLD_3rd);
...
for (int i=0; i < nData; i++){
 ...
 Int_t hit_mass_corst1_BLD_3rd = array_mass_corst1_BLD_3rd->GetEntriesFast();

if( hit_mass > 0 && hit_mass_corst1_BLD_3rd > 0){

 art::TMassData *mass = (art::TMassData*) array_mass->UncheckedAt(0);
 art::TMassData *mass_corst1_BLD_3rd = (art::TMassData*) array_mass_corst1_BLD_3rd->UncheckedAt(0);
// art::TEventHeader *eventheader = (art::TEventHeader*) array_eventheader->UncheckedAt(0);

Double_t z_select = mass->GetZ();
Double_t aq_select = mass_corst1_BLD_3rd->GetAq();
...
//  file->Close();
 ...
 tree_sc->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step.eventstore.170272.sc.root");
// tree_sc->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step_unit.eventstore.170272.sc.root");
...
for (int i=0; i < nData_sc; i++){
 ...
 Int_t hit_mass_corst1_BLD_3rd_sc = array_mass_corst1_BLD_3rd_sc->GetEntriesFast();

if( hit_mass_sc > 0 && hit_mass_corst1_BLD_3rd_sc > 0){

 art::TMassData *mass_sc = (art::TMassData*) array_mass_sc->UncheckedAt(0);
 art::TMassData *mass_corst1_BLD_3rd_sc = (art::TMassData*) array_mass_corst1_BLD_3rd_sc->UncheckedAt(0);
// art::TEventHeader *eventheader_sc = (art::TEventHeader*) array_eventheader_sc->UncheckedAt(0);

 Double_t z_select_sc = mass_sc->GetZ();
 Double_t aq_select_sc = mass_corst1_BLD_3rd_sc->GetAq();
...
//   file_sc->Close();  
 ...
 tree_ti->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step.eventstore.170272.ti.root");
// tree_ti->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step_unit.eventstore.170272.ti.root");
...
for (int i=0; i < nData_ti; i++){
 ...
 Int_t hit_mass_corst1_BLD_3rd_ti = array_mass_corst1_BLD_3rd_ti->GetEntriesFast();

if( hit_mass_ti > 0 && hit_mass_corst1_BLD_3rd_ti > 0){

 art::TMassData *mass_ti = (art::TMassData*) array_mass_ti->UncheckedAt(0);
 art::TMassData *mass_corst1_BLD_3rd_ti = (art::TMassData*) array_mass_corst1_BLD_3rd_ti->UncheckedAt(0);
// art::TEventHeader *eventheader_ti = (art::TEventHeader*) array_eventheader_ti->UncheckedAt(0);

 Double_t z_select_ti = mass_ti->GetZ();
 Double_t aq_select_ti = mass_corst1_BLD_3rd_ti->GetAq();
...
TFile* file_load_v = TFile::Open("sh13_analysis/hanai/phys/bld_file/physics.bld.170272.all.root");
 ...
 tree_v->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step.eventstore.170272.v.root");
// tree_v->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step_unit.eventstore.170272.v.root");
...
// Int_t hit_mass_v = array_mass_v->GetEntriesFast();
 Int_t hit_mass_corst1_BLD_3rd_v = array_mass_corst1_BLD_3rd_v->GetEntriesFast();

//if( hit_mass_v > 0 && hit_mass_corst1_BLD_3rd_v > 0){
if( hit_mass_corst1_BLD_3rd_v > 0){

// art::TMassData *mass_v = (art::TMassData*) array_mass_v->UncheckedAt(0);
 art::TMassData *mass_corst1_BLD_3rd_v = (art::TMassData*) array_mass_corst1_BLD_3rd_v->UncheckedAt(0);
// art::TEventHeader *eventheader_v = (art::TEventHeader*) array_eventheader_v->UncheckedAt(0);

// Double_t z_select_v = mass_v->GetZ();
 Double_t aq_select_v = mass_corst1_BLD_3rd_v->GetAq();
...
TFile* file_load_cr = TFile::Open("sh13_analysis/hanai/phys/bld_file/physics.bld.170272.all.root");
 ...
 tree_cr->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step.eventstore.170272.cr.root");
// tree_cr->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step_unit.eventstore.170272.cr.root");
...
for (int i=0; i < nData_cr; i++){
 ...
 Int_t hit_mass_corst1_BLD_3rd_cr = array_mass_corst1_BLD_3rd_cr->GetEntriesFast();

if( hit_mass_cr > 0 && hit_mass_corst1_BLD_3rd_cr > 0){

 art::TMassData *mass_cr = (art::TMassData*) array_mass_cr->UncheckedAt(0);
 art::TMassData *mass_corst1_BLD_3rd_cr = (art::TMassData*) array_mass_corst1_BLD_3rd_cr->UncheckedAt(0);
// art::TEventHeader *eventheader_cr = (art::TEventHeader*) array_eventheader_cr->UncheckedAt(0);

 Double_t z_select_cr = mass_cr->GetZ();
 Double_t aq_select_cr = mass_corst1_BLD_3rd_cr->GetAq();
...
TFile* file_load_mn = TFile::Open("sh13_analysis/hanai/phys/bld_file/physics.bld.170272.all.root");
 ...
 tree_mn->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step.eventstore.170272.mn.root");
// tree_mn->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step_unit.eventstore.170272.mn.root");
...
for (int i=0; i < nData_mn; i++){
 ...
 Int_t hit_mass_corst1_BLD_3rd_mn = array_mass_corst1_BLD_3rd_mn->GetEntriesFast();

if( hit_mass_mn > 0 && hit_mass_corst1_BLD_3rd_mn > 0){

 art::TMassData *mass_mn = (art::TMassData*) array_mass_mn->UncheckedAt(0);
 art::TMassData *mass_corst1_BLD_3rd_mn = (art::TMassData*) array_mass_corst1_BLD_3rd_mn->UncheckedAt(0);
// art::TEventHeader *eventheader_mn = (art::TEventHeader*) array_eventheader_mn->UncheckedAt(0);

 Double_t z_select_mn = mass_mn->GetZ();
 Double_t aq_select_mn = mass_corst1_BLD_3rd_mn->GetAq();
...
TFile* file_load_fe = TFile::Open("sh13_analysis/hanai/phys/bld_file/physics.bld.170272.all.root");
 ...
 tree_fe->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step.eventstore.170272.fe.root");
// tree_fe->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step_unit.eventstore.170272.fe.root");
...
for (int i=0; i < nData_fe; i++){
 ...
 Int_t hit_mass_corst1_BLD_3rd_fe = array_mass_corst1_BLD_3rd_fe->GetEntriesFast();

if( hit_mass_fe > 0 && hit_mass_corst1_BLD_3rd_fe > 0){

 art::TMassData *mass_fe = (art::TMassData*) array_mass_fe->UncheckedAt(0);
 art::TMassData *mass_corst1_BLD_3rd_fe = (art::TMassData*) array_mass_corst1_BLD_3rd_fe->UncheckedAt(0);
// art::TEventHeader *eventheader_fe = (art::TEventHeader*) array_eventheader_fe->UncheckedAt(0);

 Double_t z_select_fe = mass_fe->GetZ();
 Double_t aq_select_fe = mass_corst1_BLD_3rd_fe->GetAq();
...
TFile* file_load_co = TFile::Open("sh13_analysis/hanai/phys/bld_file/physics.bld.170272.all.root");
 ...
 tree_co->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step.eventstore.170272.co.root");
// TFile* file_co = TFile::Open("sh13_analysis/hanai/phys/merge/physics.chkmass_st1_2step.170272.co.root");

 TClonesArray *array_mass_co = NULL;
 TClonesArray *array_mass_corst1_BLD_3rd_co = NULL;
 art::TEventHeader *array_eventheader_co= NULL;
// TTree *tree_co = (TTree*) gDirectory->Get("tree");

 tree_co->SetBranchAddress("mass",&array_mass_co);
 tree_co->SetBranchAddress("mass_corst1_BLD_3rd",&array_mass_corst1_BLD_3rd_co);
 tree_co->SetBranchAddress("eventheader",&array_eventheader_co);
 tree_co->SetBranchStatus("*",0);
 tree_co->SetBranchStatus("mass",1);
 tree_co->SetBranchStatus("mass_corst1_BLD_3rd",1);
...
for (int i=0; i < nData_co; i++){
 ...
 Int_t hit_mass_corst1_BLD_3rd_co = array_mass_corst1_BLD_3rd_co->GetEntriesFast();

if( hit_mass_co > 0 && hit_mass_corst1_BLD_3rd_co > 0){

 art::TMassData *mass_co = (art::TMassData*) array_mass_co->UncheckedAt(0);
 art::TMassData *mass_corst1_BLD_3rd_co = (art::TMassData*) array_mass_corst1_BLD_3rd_co->UncheckedAt(0);
// art::TEventHeader *eventheader_fe = (art::TEventHeader*) array_eventheader_fe->UncheckedAt(0);

 Double_t z_select_co = mass_co->GetZ();
 Double_t aq_select_co = mass_corst1_BLD_3rd_co->GetAq();
...
TFile* file_load_ni = TFile::Open("sh13_analysis/hanai/phys/bld_file/physics.bld.170272.all.root");
 ...
 tree_ni->AddFriend("tree1 = tree","sh13_analysis/hanai/phys/mass_calib/physics.chkmass_st1_3step.eventstore.170272.ni.root");
// TFile* file_ni = TFile::Open("sh13_analysis/hanai/phys/merge/physics.chkmass_st1_2step.170272.ni.root");

 TClonesArray *array_mass_ni = NULL;
 TClonesArray *array_mass_corst1_BLD_3rd_ni = NULL;
 art::TEventHeader *array_eventheader_ni= NULL;
// TTree *tree_ni = (TTree*) gDirectory->Get("tree");

 tree_ni->SetBranchAddress("mass",&array_mass_ni);
 tree_ni->SetBranchAddress("mass_corst1_BLD_3rd",&array_mass_corst1_BLD_3rd_ni);
 tree_ni->SetBranchAddress("eventheader",&array_eventheader_ni);
 tree_ni->SetBranchStatus("*",0);
 tree_ni->SetBranchStatus("mass",1);
 tree_ni->SetBranchStatus("mass_corst1_BLD_3rd",1);
...
for (int i=0; i < nData_ni; i++){
 ...
 Int_t hit_mass_corst1_BLD_3rd_ni = array_mass_corst1_BLD_3rd_ni->GetEntriesFast();

if( hit_mass_ni > 0 && hit_mass_corst1_BLD_3rd_ni > 0){

 art::TMassData *mass_ni = (art::TMassData*) array_mass_ni->UncheckedAt(0);
 art::TMassData *mass_corst1_BLD_3rd_ni = (art::TMassData*) array_mass_corst1_BLD_3rd_ni->UncheckedAt(0);
// art::TEventHeader *eventheader_fe = (art::TEventHeader*) array_eventheader_fe->UncheckedAt(0);

 Double_t z_select_ni = mass_ni->GetZ();
 Double_t aq_select_ni = mass_corst1_BLD_3rd_ni->GetAq();
...
//   file_ni->Close();  
 ...
 TFile *ofile = new TFile("sh13_analysis/hanai/phys/merge/mass_corst1_BLD_3rd_all.170272.hist.root","recreate");
// TFile *ofile = new TFile("sh13_analysis/hanai/phys/merge/mass_corst1_BLD_3rd_unit_all.170272.hist.root","recreate");
