...
// get information of 39Ca
 ...
 if(!file0){
  ...
  TH1D *hca = (TH1D*)gROOT->FindObject(Form("plaq39ca_%d",irun));
  Double_t Nent_ca = hca->GetEntries();
  if (Nent_ca < 200 ) return;
...
// get information of 41Ti
 ...
 if(!file1){
  ...
  gROOT->ProcessLine("cd 0");
  TH1D *hti = (TH1D*)gROOT->FindObject(Form("plaq41ti_%d",irun));
  Double_t Nent_ti = hti->GetEntries();
  if (Nent_ti < 200 ) return;
...
// get information of 44V
 ...
 if(!file2){
  ...
  gROOT->ProcessLine("cd 0");
  TH1D *hv = (TH1D*)gROOT->FindObject(Form("plaq44v_%d",irun));
  Double_t Nent_v = hv->GetEntries();
  if (Nent_v < 200 ) return;
...
// get information of 45Cr
 ...
 if(!file3){
  ...
  gROOT->ProcessLine("cd 0");
  TH1D *hcr = (TH1D*)gROOT->FindObject(Form("plaq45cr_%d",irun));
  Double_t Nent_cr = hcr->GetEntries();
  if (Nent_cr < 200 ) return;
...
// get information of 49Fe
 ...
 if(!file4){
  ...
  gROOT->ProcessLine("cd 0");
  TH1D *hfe = (TH1D*)gROOT->FindObject(Form("plaq49fe_%d",irun));
  Double_t Nent_fe = hfe->GetEntries();
  if (Nent_fe < 200 ) return;
...
setparam(irun,mean,error,caution);
...
for(Int_t irun = 171; irun < 273; irun++){
