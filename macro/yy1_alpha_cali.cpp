...
void yy1_alpha_cali(TH2F *hin)
{
    ofstream myfile;
    myfile.open("test/yy1_cali_para.txt");
    std::vector<double> Ealpha = {3.18269, 5.48556, 5.80477};
    std::vector<double> Ealpha2 = {5.48556, 5.80477};
    ...
    for (int i = 1; i <= N; i++)
    {
        ...
        else if (i == 28)
        {
           peakposition = spec->GetPositionX();
            ...
            g[i] = new TGraphErrors(2, &ch[0], &Ealpha2[0],&ch_error[0],NULL);
            g[i]->SetMarkerColor(4);
            g[i]->SetMarkerStyle(21);
            g[i]->Draw("ap");
            f3->SetParameters(0.5,0.001);
            ...
            Npeak = spec->Search(h[i], 0, "nomarkov", 0.25);
             peakposition = spec->GetPositionX();
            //std::sort(peakposition,peakposition + 3);
            count = spec->GetPositionY();
            // std::sort(count, count + 3);
            for (int j = 1; j<=3; j++){
                ...
                f1->SetParameter(3*j-2,std::max(peakposition[0],peakposition[1])+150);
                f1->SetParLimits(3*j-2,std::max(peakposition[0],peakposition[1])+100,std::max(peakposition[0],peakposition[1])+200);
                f1->SetParameter(3*j-1,10);
                f1->SetParLimits(3*j-1,0,50);
                }
            }
