...
//// Load a input file
 ...
 TMatrixD value(Nevt,9); // output the fitting value 


 TFile* fbuff = new TFile("output/mass/mass_calib_mlp_y0_anntree.root","recreate");
 TNtuple* anntree = new TNtuple("anntree","anntree","c0:c1:c2:c3:c4:c5:c6:f");
...
for (int iref = 0; iref < Nref; iref++){
  ...
  for(int i = 0; i < nData; i++){
     ...
     if ( hit_mass > 0 && hit_tracku > 0 && hit_s0x > 0 && hit_s0y > 0){
...
// unit orientation
     ...
     anntree->Fill(c0,c1,c2,c3,c4,c5,c6,f);
...
// ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_mlp_y0.dat");
// if (foutselect.fail()){
//	 cout << "Error; Could not open output file.." << endl << endl;
//	 return;
//  }
//
//  foutselect << "t" << "  " << " | " << "x3" << "  " << " | " << "a3" << "  " << " | " << "y3" << "  " << " | " << "b3" << "  " << " | " << "x0" << "  " << " | " << "y0" << "  " << " | " << "\n";
//
//
...
//// Set ANN
   ...
   TMultiLayerPerceptron* mlp = new TMultiLayerPerceptron("c0,c1,c2,c3,c4,c5,c6:16:12:f", anntree,  "Entry$%2","(Entry$%2)==0");
   ...
   cout << "Traning ends. Generating histgrams. " << endl;

//   mlp->Export("/home/sh13/art_analysis/user/hanai/output/mass/NNfunction","C++");
...
//// Check results for selected events
 ...
 TVectorD mass_calib_sol(Nevt);

 for (int isol = 0; isol < Nevt; isol++){

     Double_t c0_tmp_sol = value(isol,2);
     Double_t c1_tmp_sol = value(isol,3);
     Double_t c2_tmp_sol = value(isol,4);
     Double_t c3_tmp_sol = value(isol,5);
     Double_t c4_tmp_sol = value(isol,6);
     Double_t c5_tmp_sol = value(isol,7);
     Double_t c6_tmp_sol = value(isol,8);

     Double_t mlp_prm_sol[7];
     mlp_prm_sol[0] = c0_tmp_sol;
     mlp_prm_sol[1] = c1_tmp_sol;
     mlp_prm_sol[2] = c2_tmp_sol;
     mlp_prm_sol[3] = c3_tmp_sol;
     mlp_prm_sol[4] = c4_tmp_sol;
     mlp_prm_sol[5] = c5_tmp_sol;
     mlp_prm_sol[6] = c6_tmp_sol;

     mass_calib_sol(isol) = mlp->Evaluate(0,mlp_prm_sol);

}

 ofstream fouteval("/home/sh13/art_analysis/user/hanai/prm/mass/evaluate_mlp_y0.dat");
 if (fouteval.fail()){
	 cout << "Error; Could not open output file.." << endl << endl;
	 return;
  }
...
for(int n = 0; n < Nevt; n++){
 ...
 for (int iisol = 0; iisol < Nevt; iisol++){
...
//     mlp_prm_sol[5] = c5_tmp_sol;
//
//     Double_t mass_calib_sol = mlp->Evaluate(0,mlp_prm_sol);

	h_aq_sol->Fill(mass_calib_sol(iisol));

	if(0 <= iisol && iisol  < 2500){
	 h_39Ca_sol->Fill(mass_calib_sol(iisol));
 } else if(2500 <= iisol && iisol  < 5000){
	 h_40Ca_sol->Fill(mass_calib_sol(iisol));
 } else if(5000 <= iisol && iisol  < 7500){
	 h_41Ca_sol->Fill(mass_calib_sol(iisol));
 } else if(7500 <= iisol && iisol  < 10000){
	 h_40Sc_sol->Fill(mass_calib_sol(iisol));
 } else if(10000 <= iisol && iisol  < 12500){
	 h_41Sc_sol->Fill(mass_calib_sol(iisol));
 } else if(12500 <= iisol && iisol  < 15000){
	 h_43Sc_sol->Fill(mass_calib_sol(iisol));
 } else if(15000 <= iisol && iisol  < 17500){
	 h_40Ti_sol->Fill(mass_calib_sol(iisol));
 } else if(17500 <= iisol && iisol  < 20000){
	 h_41Ti_sol->Fill(mass_calib_sol(iisol));
 } else if(20000 <= iisol && iisol  < 22500){
	 h_43Ti_sol->Fill(mass_calib_sol(iisol));
 } else if(22500 <= iisol && iisol  < 25000){
	 h_43V_sol->Fill(mass_calib_sol(iisol));
 } else if(25000 <= iisol && iisol  < 27500){
	 h_44V_sol->Fill(mass_calib_sol(iisol));
 } else if(27500 <= iisol && iisol  < 30000){
	 h_45V_sol->Fill(mass_calib_sol(iisol));
 } else if(30000 <= iisol && iisol  < 32500){
	 h_44Cr_sol->Fill(mass_calib_sol(iisol));
 } else if(32500 <= iisol && iisol  < 35000){
	 h_45Cr_sol->Fill(mass_calib_sol(iisol));
 } else if(35000 <= iisol && iisol  < 37500){
	 h_46Cr_sol->Fill(mass_calib_sol(iisol));
 } else if(37500 <= iisol && iisol  < 40000){
	 h_46Mn_sol->Fill(mass_calib_sol(iisol));
 } else if(40000 <= iisol && iisol  < 42500){
	 h_47Mn_sol->Fill(mass_calib_sol(iisol));
 } else if(42500 <= iisol && iisol  < 45000){
	 h_49Mn_sol->Fill(mass_calib_sol(iisol));
 } else if(45000 <= iisol && iisol  < 47500){
	 h_48Fe_sol->Fill(mass_calib_sol(iisol));
 } else if(47500 <= iisol && iisol  < 50000){
	 h_49Fe_sol->Fill(mass_calib_sol(iisol));
 } else if(50000 <= iisol && iisol  < 52500){
	 h_50Fe_sol->Fill(mass_calib_sol(iisol));
 }
}
...
//// Check results for all events
 ...
 TH1D *h_50Fe_cor = new TH1D("h50Fe_cor", "h50Fe_cor",500,1.90,1.940);
  ...
  for(int iData = 0; iData < nData; iData++){
	 ...
	 art::TSRPPACPlaneData *sr0x_tmp = (art::TSRPPACPlaneData*) array_s0x->UncheckedAt(0);
     ...
     Double_t x0i_tmp = sr0x_tmp->GetPosition();
     Double_t y0i_tmp = sr0y_tmp->GetPosition();
// unit orientation
     Double_t t_tmp = (ti_tmp - 2042.) * 0.25;  
     Double_t x3_tmp = x3i_tmp * 1.; 
     Double_t a3_tmp = a3i_tmp * 1.;
     Double_t y3_tmp = y3i_tmp * 1;
     Double_t b3_tmp = b3i_tmp * 1.;
     Double_t x0_tmp = x0i_tmp * 1.; 
     Double_t y0_tmp = y0i_tmp * 1.; 
...
//// Draw original histogram
 ...
 TFile *ofile = new TFile("output/mass/mass_calib_mlp_y0.hist.root","recreate");
