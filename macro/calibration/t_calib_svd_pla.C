...
void t_calib_svd_pla(){

 TFile* file = TFile::Open("sh13_analysis/hanai/phys/bld_file/BLD_tpla/BLD_tpla.170272.all.root");
 if(!file){
...
//  const Int_t Nrun = 250;  //the number of runs
 ...
 TClonesArray *array_mass_tpla = NULL;
...
// TTree *tree = (TTree*) gDirectory->Get("tree");
 ...
 tree->SetBranchAddress("mass_tpla",&array_mass_tpla);
 ...
 tree->SetBranchStatus("mass_tpla",1);
...
// ifstream fin("/home/sh13/art_analysis/user/hanai/prm/mass/input_tpla.dat");
...
for (int iref = 0; iref < Nref; iref++){
  ...
  for(int i = 0; i < nData; i++){
      ...
      Int_t hit_mass_tpla = array_mass_tpla->GetEntriesFast();
      Int_t hit_tracku = array_tracku->GetEntriesFast();
      Int_t hit_s0 = array_s0->GetEntriesFast();

     if ( hit_mass > 0 && hit_mass_corst1 > 0 && hit_mass_tpla > 0 && hit_tracku > 0 && hit_s0 >0){
	 ...
	 art::TMassData *mass_tpla_tmp = (art::TMassData*) array_mass_tpla->UncheckedAt(0);
	 Double_t tof_tmp = mass_tpla_tmp->GetTof();
	 Bool_t tof_ok = !TMath::IsNaN(tof_tmp);

	 art::TSRPPACPlaneData *sr0_tmp = (art::TSRPPACPlaneData*) array_s0->UncheckedAt(0);
	 Double_t s0x_tmp = sr0_tmp->GetPosition();
	 Bool_t s0x_ok = !TMath::IsNaN(s0x_tmp);


    if(Z_low[iref] < z_tmp && z_tmp < Z_up[iref] 
    	 && Aq_low[iref] < aq_tmp && aq_tmp < Aq_up[iref] && tracku_ok && tof_ok && tof_ok && s0x_ok)
    {
      irandm.push_back(i);
      n_tmp++;
     }else {
...
//// Fill data in matrix for each Ref	 
   for(int p = 0; p < Nrun; p++){
	 ...
	 art::TMassData *mass_tpla = (art::TMassData*) array_mass_tpla->UncheckedAt(0);
     ...
     Double_t aq_select = mass_tpla->GetAq();
 
     Double_t ti = mass_tpla->GetTof();
...
// ofstream fout("/home/sh13/art_analysis/user/hanai/prm/mass/set_matrix_svd3rd_tpla.dat");
 ofstream fout("/home/sh13/art_analysis/user/hanai/prm/mass/set_matrix_svd3rd_tpla_corst1select.dat");
 if (fout.fail()){
...
// ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd3rd_tpla.dat");
 ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd3rd_tpla_corst1select.dat");
 if (foutselect.fail()){
	 cout << "Error; Could not open output file.." << endl << endl;
	 return;
  }
...
//// Save the results of SVD
  ...
  Theta.Print();
   ...
   char foutname[50];
   std::ofstream foutmtr("output/mass/svd3rd_out_tpla_corst1select.dat");
   ...
   std::ofstream foutmtrth("output/mass/theta_tpla_corst1select.dat");
 if (foutmtrth.fail()){
...
//// AoQ cor
  ...
  for (Int_t iData = 0; iData < nData; iData++){
    double sum = 0.;
     ...
     tree->GetEntry(iData);
	 art::TMassData *mass_cor = (art::TMassData*) array_mass_tpla->UncheckedAt(0);
    ...
    Int_t hit_mass_cor = array_mass_tpla->GetEntriesFast();
...
//// Draw original histogram
 ...
 TH2D *h_zaq_org = new TH2D("zaq_org", "zaq_org",500,1.7,2.3,500,18,30);
  ...
  for (Int_t iData = 0; iData < nData; iData++){
   ...
   TFile *ofile = new TFile("output/mass/mass_calib_svd_3rd_tpla_corst1select.hist.root","recreate");
