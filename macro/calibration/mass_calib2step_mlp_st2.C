...
void mass_calib2step_mlp_st2(){

 TFile* file = TFile::Open("sh13_analysis/hanai/phys/merge/physics.chkmass_st2.170272.all.root");
 if(!file){
...
for (int iref = 0; iref < Nref; iref++){
  ...
  for(int i = 0; i < nData; i++){
     ...
     if ( hit_mass > 0 && hit_mass_corst2 > 0 && hit_tracku > 0 && hit_s0 > 0){

	 art::TMassData *mass_tmp = (art::TMassData*) array_mass->UncheckedAt(0);
	 art::TMassData *mass_corst2_tmp = (art::TMassData*) array_mass_corst2->UncheckedAt(0);
     Double_t z_tmp = mass_tmp->GetZ();
     Double_t aq_tmp = mass_corst2_tmp->GetAq();
...
//// Fill data in matrix for each Ref	 
   for(int p = 0; p < Nrun; p++){
     ...
     Double_t ti = dia_tof32st2->GetTiming();
...
// unit orientation
 ...
 ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_mlp_eachstripst2_ref1_2step.dat");
 if (foutselect.fail()){
...
//// Check results for selected events
 ...
 } else if(2000 <= isol && isol  < 4000){
	 h_40Ca_sol->Fill(mass_calib_sol);
 } else if(4000 <= isol && isol  < 6000){
	 h_41Ca_sol->Fill(mass_calib_sol);
 } else if(6000 <= isol && isol  < 8000){
	 h_40Sc_sol->Fill(mass_calib_sol);
 } else if(8000 <= isol && isol  < 10000){
	 h_41Sc_sol->Fill(mass_calib_sol);
 } else if(10000 <= isol && isol  < 12000){
	 h_43Sc_sol->Fill(mass_calib_sol);
 } else if(12000 <= isol && isol  < 14000){
	 h_40Ti_sol->Fill(mass_calib_sol);
 } else if(14000 <= isol && isol  < 16000){
	 h_41Ti_sol->Fill(mass_calib_sol);
 } else if(16000 <= isol && isol  < 18000){
	 h_43Ti_sol->Fill(mass_calib_sol);
 } else if(18000 <= isol && isol  < 20000){
	 h_43V_sol->Fill(mass_calib_sol);
 } else if(20000 <= isol && isol  < 22000){
	 h_44V_sol->Fill(mass_calib_sol);
 } else if(22000 <= isol && isol  < 24000){
	 h_45V_sol->Fill(mass_calib_sol);
 } else if(24000 <= isol && isol  < 26000){
	 h_44Cr_sol->Fill(mass_calib_sol);
 } else if(26000 <= isol && isol  < 28000){
	 h_45Cr_sol->Fill(mass_calib_sol);
 } else if(28000 <= isol && isol  < 30000){
	 h_46Cr_sol->Fill(mass_calib_sol);
 } else if(30000 <= isol && isol  < 32000){
	 h_46Mn_sol->Fill(mass_calib_sol);
 } else if(32000 <= isol && isol  < 34000){
	 h_47Mn_sol->Fill(mass_calib_sol);
 } else if(34000 <= isol && isol  < 36000){
	 h_48Mn_sol->Fill(mass_calib_sol);
 } else if(36000 <= isol && isol  < 38000){
	 h_48Fe_sol->Fill(mass_calib_sol);
 } else if(38000 <= isol && isol  < 40000){
	 h_49Fe_sol->Fill(mass_calib_sol);
 } else if(40000 <= isol && isol  < 42000){
...
// Output
 ...
 TH1D *h_39Ca_cor = new TH1D("h39Ca_cor", "h39Ca_cor",1000,1.940,1.960);
 TH1D *h_40Ca_cor = new TH1D("h40Ca_cor", "h40Ca_cor",1000,1.990,2.01);
 TH1D *h_41Ca_cor = new TH1D("h41Ca_cor", "h41Ca_cor",1000,2.040,2.060);
                               
 TH1D *h_40Sc_cor = new TH1D("h40Sc_cor", "h40Sc_cor",1000,1.900,1.92);
 TH1D *h_41Sc_cor = new TH1D("h41Sc_cor", "h41Sc_cor",1000,1.945,1.965);
 TH1D *h_43Sc_cor = new TH1D("h43Sc_cor", "h43Sc_cor",1000,2.04,2.060);
                              
 TH1D *h_40Ti_cor = new TH1D("h40Ti_cor", "h40Ti_cor",1000,1.81,1.83);
 TH1D *h_41Ti_cor = new TH1D("h41Ti_cor", "h41Ti_cor",1000,1.86,1.88);
 TH1D *h_43Ti_cor = new TH1D("h43Ti_cor", "h43Ti_cor",1000,1.940,1.960);
                                                            
 TH1D *h_43V_cor  = new TH1D("h43V_cor", "h43V_cor",1000,1.86,1.880);
 TH1D *h_44V_cor  = new TH1D("h44V_cor", "h44V_cor",1000,1.90,1.92);
 TH1D *h_45V_cor  = new TH1D("h45V_cor", "h45V_cor",1000,1.945,1.965);
                                                            
 TH1D *h_44Cr_cor = new TH1D("h44Cr_cor", "h44Cr_cor",1000,1.83,1.85);
 TH1D *h_45Cr_cor = new TH1D("h45Cr_cor", "h45Cr_cor",1000,1.865,1.885);
 TH1D *h_46Cr_cor = new TH1D("h46Cr_cor", "h46Cr_cor",1000,1.90,1.925);
                              
 TH1D *h_46Mn_cor = new TH1D("h46Mn_cor", "h46Mn_cor",1000,1.83,1.85);
 TH1D *h_47Mn_cor = new TH1D("h47Mn_cor", "h47Mn_cor",1000,1.87,1.897);
 TH1D *h_48Mn_cor = new TH1D("h48Mn_cor", "h48Mn_cor",1000,1.91,1.930);
                               
 TH1D *h_48Fe_cor = new TH1D("h48Fe_cor", "h48Fe_cor",1000,1.835,1.855);
 TH1D *h_49Fe_cor = new TH1D("h49Fe_cor", "h49Fe_cor",1000,1.87,1.89);
 TH1D *h_50Fe_cor = new TH1D("h50Fe_cor", "h50Fe_cor",1000,1.91,1.930);
 ...
 TH2D *h_zaq_cor = new TH2D("zaq_col", "zaq_col",500,1.7,2.3,500,18,30);
  ...
  for (Int_t iData = 0; iData < nData; iData++){
      ...
      Int_t hit_dia_tof32st2 = array_dia_tof32st2->GetEntriesFast();
      Int_t hit_mass = array_mass->GetEntriesFast();
      Int_t hit_tracku = array_tracku->GetEntriesFast();
      Int_t hit_s0 = array_s0->GetEntriesFast();

     if ( hit_dia_tof32st2 > 0 && hit_tracku > 0 && hit_mass > 0 && hit_s0 >0){

	 art::TMassData *mass_tmp = (art::TMassData*) array_mass->UncheckedAt(0);
	 art::TDiaTimingData *dia_tof32st2_tmp = (art::TDiaTimingData*) array_dia_tof32st2->UncheckedAt(0);
     Double_t z_tmp = mass_tmp->GetZ();
     ...
     Double_t ti_tmp = dia_tof32st2_tmp->GetTiming();
...
// unit orientation
     ...
     h_zaq_cor->Fill(mass_calib,z_tmp); 

for(int iref = 0; iref < Nref; iref++){
      if(Z_low[iref] < z_tmp && z_tmp < Z_up[iref]  && tracku_ok)
    {
      switch(iref){
		  case 0:
...
//// Draw original histogram
 ...
 TH2D *h_zaq_org = new TH2D("zaq_org", "zaq_org",500,1.7,2.3,500,18,30);
  ...
  for (Int_t iData = 0; iData < nData; iData++){
     ...
     art::TMassData *mass_corst2_tmpt = (art::TMassData*) array_mass_corst2->UncheckedAt(0);
     Double_t z_tmpt = mass_tmpt->GetZ();
     Double_t aq_tmpt = mass_corst2_tmpt->GetAq();
   ...
   TFile *ofile = new TFile("output/mass/mass_calib_mlp_3rd_eachstrip_st2_ref1_2step.hist.root","recreate");
