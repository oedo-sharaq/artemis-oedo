...
void t_calib_svd_st3(){

// TFile* file = TFile::Open("sh13_analysis/hanai/phys/merge/physics.chkAQ.170272.all.root");
...
//  const Int_t Nrun = 250;  //the number of runs
 ...
 ifstream fin("/home/sh13/art_analysis/user/hanai/prm/mass/input_try_st3.dat");
 if (fin.fail()){
...
for (int iref = 0; iref < Nref; iref++){
  ...
  for(int i = 0; i < nData; i++){
     ...
     if ( hit_mass > 0 && hit_mass_st3 > 0 && hit_tracku > 0 && hit_s0 >0){

	 art::TMassData *mass_tmp = (art::TMassData*) array_mass_st3->UncheckedAt(0);
...
// ofstream fout("/home/sh13/art_analysis/user/hanai/prm/mass/set_matrix_svd3rd_eachstripst3.dat");
...
// ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd3rd_eachstripst3.dat");
...
//// Save the results of SVD
  ...
  Theta.Print();
   ...
   char foutname[50];
//   std::ofstream foutmtr("output/mass/svd3rd_out_st3.dat");
 ...
 //  std::ofstream foutmtrth("output/mass/theta_st3.dat");
...
//// AoQ cor
  ...
  for (Int_t iData = 0; iData < nData; iData++){
    double sum = 0.;
     ...
     tree->GetEntry(iData);
	 art::TMassData *mass_cor = (art::TMassData*) array_mass_st3->UncheckedAt(0);
    ...
    Int_t hit_mass_cor = array_mass_st3->GetEntriesFast();
...
//   TFile *ofile = new TFile("output/mass/mass_calib_svd_3rd_eachstrip_st3.hist.root","recreate");
