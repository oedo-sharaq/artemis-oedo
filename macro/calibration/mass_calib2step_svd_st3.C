...
void mass_calib2step_svd_st3(){

 TFile* file = TFile::Open("sh13_analysis/hanai/phys/merge/physics.chkmass_st3.170272.all.root");
 if(!file){
...
//  const Int_t Nrun = 2500;  //the number of runs
 ...
 tree->SetBranchAddress("mass_corst3",&array_mass_corst3);
 ...
 tree->SetBranchStatus("mass_corst3",1);
 ...
 ifstream fin("/home/sh13/art_analysis/user/hanai/prm/mass/input_2step_st3.dat");
 if (fin.fail()){
...
for (int iref = 0; iref < Nref; iref++){
  ...
  for(int i = 0; i < nData; i++){
     ...
     if ( hit_mass > 0 && hit_mass_corst3 > 0 && hit_tracku > 0 && hit_s0 > 0){
...
// unit orientation
 ...
 ofstream fout("/home/sh13/art_analysis/user/hanai/prm/mass/set_matrix_svd3rd_eachstripst3_ref1_2step.dat");
 if (fout.fail()){
...
for(int n = 0; n < Nevt; n++){
 ...
 ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd3rd_eachstripst3_ref1_2step.dat");
 if (foutselect.fail()){
	 cout << "Error; Could not open output file.." << endl << endl;
	 return;
  }
...
//// Save the results of SVD
  ...
  Theta.Print();
   ...
   char foutname[50];
   std::ofstream foutmtr("output/mass/svd3rd_out_st3_ref1_2step.dat");
   ...
   std::ofstream foutmtrth("output/mass/theta_st3_ref1_2step.dat");
 if (foutmtrth.fail()){
...
//// AoQ cor
  ...
  for (Int_t iData = 0; iData < nData; iData++){
    ...
    Int_t hit_s0_cor = array_s0->GetEntriesFast();
     ...
     if ( hit_dia_tof32st3cor > 0 && hit_tracku_cor > 0 && hit_s0_cor >0){
...
//// Draw original histogram
 ...
 TH2D *h_zaq_org = new TH2D("zaq_org", "zaq_org",500,1.7,2.3,500,18,30);
  ...
  for (Int_t iData = 0; iData < nData; iData++){
   ...
   TFile *ofile = new TFile("output/mass/mass_calib_svd_3rd_eachstrip_st3_ref1_2step.hist.root","recreate");
