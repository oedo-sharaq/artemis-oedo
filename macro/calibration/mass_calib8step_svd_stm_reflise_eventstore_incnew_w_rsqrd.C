...
void mass_calib8step_svd_stm_reflise_eventstore_incnew_w_rsqrd(){
...
//  const Int_t Nevt =  (Nref - 3) * Nrun + 2 * Nrun_casc + Nrun_ni;  // the number of events
...
// ifstream fin("/home/sh13/art_analysis/user/hanai/prm/mass/input_8step_stm_reflise.dat");
 ifstream fin("/home/sh13/art_analysis/user/hanai/prm/mass/input_8step_stm_reflise_w2.dat");
 if (fin.fail()){
  ...
  while(!fin.eof()){
	    fin.precision(10);
	    fin >> Ref_m_tmp >> Z_low_tmp >> Z_up_tmp >> Aq_low_tmp >> Aq_up_tmp >> weight_tmp;
 ...
 Double_t Z_low_draw_tmp, Z_up_draw_tmp, Aq_low_draw_tmp, Aq_up_draw_tmp;
 vector<Double_t> Z_low_draw, Z_up_draw, Aq_low_draw, Aq_up_draw;

 ifstream finoekaki("/home/sh13/art_analysis/user/hanai/pidoekaki.dat");
 if (finoekaki.fail()){
	 cout << "Error; inputfile" << " " << "is not found." << endl << endl;
	 return;
  }
  ...
  while(!finoekaki.eof()){
	    finoekaki.precision(10);
	    finoekaki >> Z_low_draw_tmp >> Z_up_draw_tmp >> Aq_low_draw_tmp >> Aq_up_draw_tmp;
		Z_low_draw.push_back(Z_low_draw_tmp);
		Z_up_draw.push_back(Z_up_draw_tmp);
		Aq_low_draw.push_back(Aq_low_draw_tmp);
		Aq_up_draw.push_back(Aq_up_draw_tmp);
		}
 finoekaki.close();
 ...
 TMatrixD WA(Nevt,Nprm);
 TMatrixD b(Nevt,1);
 TMatrixD b_input(Nevt,1);
...
for (int iref = 0; iref < Nref; iref++){
  ...
  Double_t weightscale = scale*  weight[iref];
...
//     b(ievt,0) = Ref_m[iref];
     b(ievt,0) = Ref_m[iref] * Ref_m[iref] * weightscale;
//     b_ave += Ref_m[iref] * Ref_m[iref] / Nevt;
     b_ave += Ref_m[iref] / Nevt;
     b_input(ievt,0) = Ref_m[iref];
 
// constant
     WA(ievt,0) = 1. * weightscale; 
// 1st order
     WA(ievt,1)  = t * weightscale;
     WA(ievt,2)  = x3 * weightscale;
     WA(ievt,3)  = a3 * weightscale;
     WA(ievt,4)  = y3 * weightscale;
     WA(ievt,5)  = b3 * weightscale;
     WA(ievt,6)  = x0 * weightscale;
     WA(ievt,7)  = x2 * weightscale;
     WA(ievt,8)  = a2 * weightscale;
     WA(ievt,9)  = y2 * weightscale;
     WA(ievt,10) = b2 * weightscale;
// 2nd order       
     WA(ievt,11) = t * t * weightscale;
     WA(ievt,12) = x3 * x3 * weightscale;
     WA(ievt,13) = a3 * a3 * weightscale;
     WA(ievt,14) = y3 * y3 * weightscale;
     WA(ievt,15) = b3 * b3 * weightscale;
     WA(ievt,16) = x0 * x0 * weightscale;
     WA(ievt,17) = x2 * x2 * weightscale;
     WA(ievt,18) = a2 * a2 * weightscale;
     WA(ievt,19) = y2 * y2 * weightscale;
     WA(ievt,20) = b2 * b2 * weightscale;
 
     WA(ievt,21) = t * x3 * weightscale;
     WA(ievt,22) = t * a3 * weightscale;
     WA(ievt,23) = t * y3 * weightscale;
     WA(ievt,24) = t * b3 * weightscale;
     WA(ievt,25) = t * x0 * weightscale;
     WA(ievt,26) = t * x2 * weightscale;
     WA(ievt,27) = t * a2 * weightscale;
     WA(ievt,28) = t * y2 * weightscale;
     WA(ievt,29) = t * b2 * weightscale;
 
     WA(ievt,30) = x3 * a3 * weightscale;
     WA(ievt,31) = x3 * y3 * weightscale;
     WA(ievt,32) = x3 * b3 * weightscale;
     WA(ievt,33) = x3 * x0 * weightscale;
     WA(ievt,34) = x3 * x2 * weightscale;
     WA(ievt,35) = x3 * a2 * weightscale;
     WA(ievt,36) = x3 * y2 * weightscale;
     WA(ievt,37) = x3 * b2 * weightscale;
 
     WA(ievt,38) = a3 * y3 * weightscale;
     WA(ievt,39) = a3 * b3 * weightscale;
     WA(ievt,40) = a3 * x0 * weightscale;
     WA(ievt,41) = a3 * x2 * weightscale;
     WA(ievt,42) = a3 * a2 * weightscale;
     WA(ievt,43) = a3 * y2 * weightscale;
     WA(ievt,44) = a3 * b2 * weightscale;
 
     WA(ievt,45) = y3 * b3 * weightscale;
     WA(ievt,46) = y3 * x0 * weightscale;
     WA(ievt,47) = y3 * x2 * weightscale;
     WA(ievt,48) = y3 * a2 * weightscale;
     WA(ievt,49) = y3 * y2 * weightscale;
     WA(ievt,50) = y3 * b2 * weightscale;
 
     WA(ievt,51) = b3 * x0 * weightscale;
     WA(ievt,52) = b3 * x2 * weightscale;
     WA(ievt,53) = b3 * a2 * weightscale;
     WA(ievt,54) = b3 * y2 * weightscale;
     WA(ievt,55) = b3 * b2 * weightscale;
 
     WA(ievt,56) = x0 * x2 * weightscale;
     WA(ievt,57) = x0 * a2 * weightscale;
     WA(ievt,58) = x0 * y2 * weightscale;
     WA(ievt,59) = x0 * b2 * weightscale;
 
     WA(ievt,60) = x2 * a2 * weightscale;
     WA(ievt,61) = x2 * y2 * weightscale;
     WA(ievt,62) = x2 * b2 * weightscale;
 
     WA(ievt,63) = a2 * y2 * weightscale;
     WA(ievt,64) = a2 * b2 * weightscale;
 
     WA(ievt,65) = y2 * b2 * weightscale;
// 4th order
     WA(ievt,66) = t * t * t * weightscale;
     WA(ievt,67) = x3 * x3 * x3 * weightscale;
     WA(ievt,68) = a3 * a3 * a3 * weightscale;
     WA(ievt,69) = y3 * y3 * y3 * weightscale;
     WA(ievt,70) = b3 * b3 * b3 * weightscale;
     WA(ievt,71) = x0 * x0 * x0 * weightscale;
     WA(ievt,72) = x2 * x2 * x2 * weightscale;
     WA(ievt,73) = a2 * a2 * a2 * weightscale;
     WA(ievt,74) = y2 * y2 * y2 * weightscale;
     WA(ievt,75) = b2 * b2 * b2 * weightscale;
 
     WA(ievt,76) = t * t * x3 * weightscale;
     WA(ievt,77) = t * t * a3 * weightscale;
     WA(ievt,78) = t * t * y3 * weightscale;
     WA(ievt,79) = t * t * b3 * weightscale;
     WA(ievt,80) = t * t * x0 * weightscale;
     WA(ievt,81) = t * t * x2 * weightscale;
     WA(ievt,82) = t * t * a2 * weightscale;
     WA(ievt,83) = t * t * y2 * weightscale;
     WA(ievt,84) = t * t * b2 * weightscale;
 
     WA(ievt,85) = x3 * x3 * t * weightscale;
     WA(ievt,86) = x3 * x3 * a3 * weightscale;
     WA(ievt,87) = x3 * x3 * y3 * weightscale;
     WA(ievt,88) = x3 * x3 * b3 * weightscale;
     WA(ievt,89) = x3 * x3 * x0 * weightscale;
     WA(ievt,90) = x3 * x3 * x2 * weightscale;
     WA(ievt,91) = x3 * x3 * a2 * weightscale;
     WA(ievt,92) = x3 * x3 * y2 * weightscale;
     WA(ievt,93) = x3 * x3 * b2 * weightscale;
 
     WA(ievt,94) = a3 * a3 * t * weightscale;
     WA(ievt,95) = a3 * a3 * x3 * weightscale;
     WA(ievt,96) = a3 * a3 * y3 * weightscale;
     WA(ievt,97) = a3 * a3 * b3 * weightscale;
     WA(ievt,98) = a3 * a3 * x0 * weightscale;
     WA(ievt,99) = a3 * a3 * x2 * weightscale;
     WA(ievt,100) = a3 * a3 * a2 * weightscale;
     WA(ievt,101) = a3 * a3 * y2 * weightscale;
     WA(ievt,102) = a3 * a3 * b2 * weightscale;
 
     WA(ievt,103) = y3 * y3 * t * weightscale;
     WA(ievt,104) = y3 * y3 * x3 * weightscale;
     WA(ievt,105) = y3 * y3 * a3 * weightscale;
     WA(ievt,106) = y3 * y3 * b3 * weightscale;
     WA(ievt,107) = y3 * y3 * x0 * weightscale;
     WA(ievt,108) = y3 * y3 * x2 * weightscale;
     WA(ievt,109) = y3 * y3 * a2 * weightscale;
     WA(ievt,110) = y3 * y3 * y2 * weightscale;
     WA(ievt,111) = y3 * y3 * b2 * weightscale;
 
     WA(ievt,112) = b3 * b3 * t * weightscale;
     WA(ievt,113) = b3 * b3 * x3 * weightscale;
     WA(ievt,114) = b3 * b3 * a3 * weightscale;
     WA(ievt,115) = b3 * b3 * y3 * weightscale;
     WA(ievt,116) = b3 * b3 * x0 * weightscale;
     WA(ievt,117) = b3 * b3 * x2 * weightscale;
     WA(ievt,118) = b3 * b3 * a2 * weightscale;
     WA(ievt,119) = b3 * b3 * y2 * weightscale;
     WA(ievt,120) = b3 * b3 * b2 * weightscale;
 
     WA(ievt,121) = x0 * x0 * t * weightscale;
     WA(ievt,122) = x0 * x0 * x3 * weightscale;
     WA(ievt,123) = x0 * x0 * a3 * weightscale;
     WA(ievt,124) = x0 * x0 * y3 * weightscale;
     WA(ievt,125) = x0 * x0 * b3 * weightscale;
     WA(ievt,126) = x0 * x0 * x2 * weightscale;
     WA(ievt,127) = x0 * x0 * a2 * weightscale;
     WA(ievt,128) = x0 * x0 * y2 * weightscale;
     WA(ievt,129) = x0 * x0 * b2 * weightscale;
 
     WA(ievt,130) = x2 * x2 * t * weightscale;
     WA(ievt,131) = x2 * x2 * x3 * weightscale;
     WA(ievt,132) = x2 * x2 * a3 * weightscale;
     WA(ievt,133) = x2 * x2 * y3 * weightscale;
     WA(ievt,134) = x2 * x2 * b3 * weightscale;
     WA(ievt,135) = x2 * x2 * x0 * weightscale;
     WA(ievt,136) = x2 * x2 * a2 * weightscale;
     WA(ievt,137) = x2 * x2 * y2 * weightscale;
     WA(ievt,138) = x2 * x2 * b2 * weightscale;
 
     WA(ievt,139) = a2 * a2 * t * weightscale;
     WA(ievt,140) = a2 * a2 * x3 * weightscale;
     WA(ievt,141) = a2 * a2 * a3 * weightscale;
     WA(ievt,142) = a2 * a2 * y3 * weightscale;
     WA(ievt,143) = a2 * a2 * b3 * weightscale;
     WA(ievt,144) = a2 * a2 * x0 * weightscale;
     WA(ievt,145) = a2 * a2 * x2 * weightscale;
     WA(ievt,146) = a2 * a2 * y2 * weightscale;
     WA(ievt,147) = a2 * a2 * b2 * weightscale;
 
     WA(ievt,148) = y2 * y2 * t * weightscale;
     WA(ievt,149) = y2 * y2 * x3 * weightscale;
     WA(ievt,150) = y2 * y2 * a3 * weightscale;
     WA(ievt,151) = y2 * y2 * y3 * weightscale;
     WA(ievt,152) = y2 * y2 * b3 * weightscale;
     WA(ievt,153) = y2 * y2 * x0 * weightscale;
     WA(ievt,154) = y2 * y2 * x2 * weightscale;
     WA(ievt,155) = y2 * y2 * a2 * weightscale;
     WA(ievt,156) = y2 * y2 * b2 * weightscale;
 
     WA(ievt,157) = b2 * b2 * t * weightscale;
     WA(ievt,158) = b2 * b2 * x3 * weightscale;
     WA(ievt,159) = b2 * b2 * a3 * weightscale;
     WA(ievt,160) = b2 * b2 * y3 * weightscale;
     WA(ievt,161) = b2 * b2 * b3 * weightscale;
     WA(ievt,162) = b2 * b2 * x0 * weightscale;
     WA(ievt,163) = b2 * b2 * x2 * weightscale;
     WA(ievt,164) = b2 * b2 * a2 * weightscale;
     WA(ievt,165) = b2 * b2 * y2 * weightscale;
 
     WA(ievt,166) = t * x3 * a3 * weightscale;
     WA(ievt,167) = t * x3 * y3 * weightscale;
     WA(ievt,168) = t * x3 * b3 * weightscale;
     WA(ievt,169) = t * x3 * x0 * weightscale;
     WA(ievt,170) = t * x3 * x2 * weightscale;
     WA(ievt,171) = t * x3 * a2 * weightscale;
     WA(ievt,172) = t * x3 * y2 * weightscale;
     WA(ievt,173) = t * x3 * b2 * weightscale;
 
     WA(ievt,174) = t * a3 * y3 * weightscale;
     WA(ievt,175) = t * a3 * b3 * weightscale;
     WA(ievt,176) = t * a3 * x0 * weightscale;
     WA(ievt,177) = t * a3 * x2 * weightscale;
     WA(ievt,178) = t * a3 * a2 * weightscale;
     WA(ievt,179) = t * a3 * y2 * weightscale;
     WA(ievt,180) = t * a3 * b2 * weightscale;
 
     WA(ievt,181) = t * y3 * b3 * weightscale;
     WA(ievt,182) = t * y3 * x0 * weightscale;
     WA(ievt,183) = t * y3 * x2 * weightscale;
     WA(ievt,184) = t * y3 * a2 * weightscale;
     WA(ievt,185) = t * y3 * y2 * weightscale;
     WA(ievt,186) = t * y3 * b2 * weightscale;
 
     WA(ievt,187) = t * b3 * x0 * weightscale;
     WA(ievt,188) = t * b3 * x2 * weightscale;
     WA(ievt,189) = t * b3 * a2 * weightscale;
     WA(ievt,190) = t * b3 * y2 * weightscale;
     WA(ievt,192) = t * b3 * b2 * weightscale;
 
     WA(ievt,193) = t * x0 * x2 * weightscale;
     WA(ievt,194) = t * x0 * a2 * weightscale;
     WA(ievt,195) = t * x0 * y2 * weightscale;
     WA(ievt,196) = t * x0 * b2 * weightscale;
 
     WA(ievt,197) = t * x2 * a2 * weightscale;
     WA(ievt,198) = t * x2 * y2 * weightscale;
     WA(ievt,199) = t * x2 * b2 * weightscale;
 
     WA(ievt,200) = t * a2 * y2 * weightscale;
     WA(ievt,201) = t * a2 * b2 * weightscale;
 
     WA(ievt,202) = t * y2 * b2 * weightscale;
 
     WA(ievt,203) = x3 * a3 * y3 * weightscale;
     WA(ievt,204) = x3 * a3 * b3 * weightscale;
     WA(ievt,205) = x3 * a3 * x0 * weightscale;
     WA(ievt,206) = x3 * a3 * x2 * weightscale;
     WA(ievt,207) = x3 * a3 * a2 * weightscale;
     WA(ievt,208) = x3 * a3 * y2 * weightscale;
     WA(ievt,209) = x3 * a3 * b2 * weightscale;
 
     WA(ievt,210) = x3 * y3 * b3 * weightscale;
     WA(ievt,211) = x3 * y3 * x0 * weightscale;
     WA(ievt,212) = x3 * y3 * x2 * weightscale;
     WA(ievt,213) = x3 * y3 * a2 * weightscale;
     WA(ievt,214) = x3 * y3 * y2 * weightscale;
     WA(ievt,215) = x3 * y3 * b2 * weightscale;
 
     WA(ievt,216) = x3 * b3 * x0 * weightscale;
     WA(ievt,217) = x3 * b3 * x2 * weightscale;
     WA(ievt,217) = x3 * b3 * a2 * weightscale;
     WA(ievt,218) = x3 * b3 * y2 * weightscale;
     WA(ievt,219) = x3 * b3 * b2 * weightscale;
 
     WA(ievt,220) = x3 * x0 * x2 * weightscale;
     WA(ievt,221) = x3 * x0 * a2 * weightscale;
     WA(ievt,222) = x3 * x0 * y2 * weightscale;
     WA(ievt,223) = x3 * x0 * b2 * weightscale;
        
     WA(ievt,224) = x3 * x2 * a2 * weightscale;
     WA(ievt,225) = x3 * x2 * y2 * weightscale;
     WA(ievt,226) = x3 * x2 * b2 * weightscale;
        
     WA(ievt,227) = x3 * a2 * y2 * weightscale;
     WA(ievt,228) = x3 * a2 * b2 * weightscale;
 
     WA(ievt,229) = x3 * y2 * b2 * weightscale;
 
     WA(ievt,230) = a3 * y3 * b3 * weightscale;
     WA(ievt,231) = a3 * y3 * x0 * weightscale;
     WA(ievt,232) = a3 * y3 * x2 * weightscale;
     WA(ievt,233) = a3 * y3 * a2 * weightscale;
     WA(ievt,234) = a3 * y3 * y2 * weightscale;
     WA(ievt,235) = a3 * y3 * b2 * weightscale;
 
     WA(ievt,236) = a3 * b3 * x0 * weightscale;
     WA(ievt,237) = a3 * b3 * x2 * weightscale;
     WA(ievt,238) = a3 * b3 * a2 * weightscale;
     WA(ievt,239) = a3 * b3 * y2 * weightscale;
     WA(ievt,240) = a3 * b3 * b2 * weightscale;
 
     WA(ievt,241) = a3 * x0 * x2 * weightscale;
     WA(ievt,242) = a3 * x0 * a2 * weightscale;
     WA(ievt,243) = a3 * x0 * y2 * weightscale;
     WA(ievt,244) = a3 * x0 * b2 * weightscale;
 
     WA(ievt,245) = a3 * x2 * a2 * weightscale;
     WA(ievt,246) = a3 * x2 * y2 * weightscale;
     WA(ievt,247) = a3 * x2 * b2 * weightscale;
 
     WA(ievt,248) = a3 * a2 * y2 * weightscale;
     WA(ievt,249) = a3 * a2 * b2 * weightscale;
 
     WA(ievt,250) = a3 * y2 * b2 * weightscale;
 
     WA(ievt,251) = y3 * b3 * x0 * weightscale;
     WA(ievt,252) = y3 * b3 * x2 * weightscale;
     WA(ievt,253) = y3 * b3 * a2 * weightscale;
     WA(ievt,254) = y3 * b3 * y2 * weightscale;
     WA(ievt,255) = y3 * b3 * b2 * weightscale;
 
     WA(ievt,256) = y3 * x0 * x2 * weightscale;
     WA(ievt,257) = y3 * x0 * a2 * weightscale;
     WA(ievt,258) = y3 * x0 * y2 * weightscale;
     WA(ievt,259) = y3 * x0 * b2 * weightscale;
 
     WA(ievt,260) = y3 * x2 * a2 * weightscale;
     WA(ievt,261) = y3 * x2 * y2 * weightscale;
     WA(ievt,262) = y3 * x2 * b2 * weightscale;
 
     WA(ievt,263) = y3 * a2 * y2 * weightscale;
     WA(ievt,264) = y3 * a2 * b2 * weightscale;
 
     WA(ievt,265) = y3 * y2 * b2 * weightscale;
 
     WA(ievt,266) = b3 * x0 * x2 * weightscale;
     WA(ievt,267) = b3 * x0 * a2 * weightscale;
     WA(ievt,268) = b3 * x0 * y2 * weightscale;
     WA(ievt,269) = b3 * x0 * b2 * weightscale;
 
     WA(ievt,270) = b3 * x2 * a2 * weightscale;
     WA(ievt,271) = b3 * x2 * y2 * weightscale;
     WA(ievt,272) = b3 * x2 * b2 * weightscale;
 
     WA(ievt,273) = b3 * a2 * y2 * weightscale;
     WA(ievt,274) = b3 * a2 * b2 * weightscale;
 
     WA(ievt,275) = b3 * y2 * b2 * weightscale;
 
     WA(ievt,276) = x0 * x2 * a2 * weightscale;
     WA(ievt,277) = x0 * x2 * y2 * weightscale;
     WA(ievt,278) = x0 * x2 * b2 * weightscale;
 
     WA(ievt,279) = x0 * a2 * y2 * weightscale;
     WA(ievt,280) = x0 * a2 * b2 * weightscale;
 
     WA(ievt,281) = x0 * y2 * b2 * weightscale;
 
     WA(ievt,282) = x2 * a2 * y2 * weightscale;
     WA(ievt,283) = x2 * a2 * b2 * weightscale;
 
     WA(ievt,284) = x2 * y2 * b2 * weightscale;
 
     WA(ievt,285) = a2 * y2 * b2 * weightscale;
// 4th order
     WA(ievt,286) = t * t * t * t * weightscale;
     WA(ievt,287) = x3 * x3 * x3 * x3 * weightscale;
     WA(ievt,288) = a3 * a3 * a3 * a3 * weightscale;
     WA(ievt,289) = y3 * y3 * y3 * y3 * weightscale;
     WA(ievt,290) = b3 * b3 * b3 * b3 * weightscale;
     WA(ievt,291) = x0 * x0 * x0 * x0 * weightscale;
     WA(ievt,292) = x2 * x2 * x2 * x2 * weightscale;
     WA(ievt,293) = a2 * a2 * a2 * a2 * weightscale;
     WA(ievt,294) = y2 * y2 * y2 * y2 * weightscale;
     WA(ievt,295) = b2 * b2 * b2 * b2 * weightscale;

     WA(ievt,296) = t * t * t * x3 * weightscale;
     WA(ievt,297) = t * t * t * a3 * weightscale;
     WA(ievt,298) = t * t * t * y3 * weightscale;
     WA(ievt,299) = t * t * t * b3 * weightscale;
     WA(ievt,300) = t * t * t * x0 * weightscale;
     WA(ievt,301) = t * t * t * x2 * weightscale;
     WA(ievt,302) = t * t * t * a2 * weightscale;
     WA(ievt,303) = t * t * t * y2 * weightscale;
     WA(ievt,304) = t * t * t * b2 * weightscale;

     WA(ievt,305) = x3 * x3 * x3 * t * weightscale;
     WA(ievt,306) = x3 * x3 * x3 * a3 * weightscale;
     WA(ievt,307) = x3 * x3 * x3 * y3 * weightscale;
     WA(ievt,308) = x3 * x3 * x3 * b3 * weightscale;
     WA(ievt,309) = x3 * x3 * x3 * x0 * weightscale;
     WA(ievt,310) = x3 * x3 * x3 * x2 * weightscale;
     WA(ievt,311) = x3 * x3 * x3 * a2 * weightscale;
     WA(ievt,312) = x3 * x3 * x3 * y2 * weightscale;
     WA(ievt,313) = x3 * x3 * x3 * b2 * weightscale;

     WA(ievt,314) = a3 * a3 * a3 * t * weightscale;
     WA(ievt,315) = a3 * a3 * a3 * x3 * weightscale;
     WA(ievt,316) = a3 * a3 * a3 * y3 * weightscale;
     WA(ievt,317) = a3 * a3 * a3 * b3 * weightscale;
     WA(ievt,318) = a3 * a3 * a3 * x0 * weightscale;
     WA(ievt,319) = a3 * a3 * a3 * x2 * weightscale;
     WA(ievt,320) = a3 * a3 * a3 * a2 * weightscale;
     WA(ievt,321) = a3 * a3 * a3 * y2 * weightscale;
     WA(ievt,322) = a3 * a3 * a3 * b2 * weightscale;

     WA(ievt,323) = y3 * y3 * y3 * t * weightscale;
     WA(ievt,324) = y3 * y3 * y3 * x3 * weightscale;
     WA(ievt,325) = y3 * y3 * y3 * a3 * weightscale;
     WA(ievt,326) = y3 * y3 * y3 * b3 * weightscale;
     WA(ievt,327) = y3 * y3 * y3 * x0 * weightscale;
     WA(ievt,328) = y3 * y3 * y3 * x2 * weightscale;
     WA(ievt,329) = y3 * y3 * y3 * a2 * weightscale;
     WA(ievt,330) = y3 * y3 * y3 * y2 * weightscale;
     WA(ievt,331) = y3 * y3 * y3 * b2 * weightscale;

     WA(ievt,332) = b3 * b3 * b3 * t * weightscale;
     WA(ievt,333) = b3 * b3 * b3 * x3 * weightscale;
     WA(ievt,334) = b3 * b3 * b3 * a3 * weightscale;
     WA(ievt,335) = b3 * b3 * b3 * y3 * weightscale;
     WA(ievt,336) = b3 * b3 * b3 * x0 * weightscale;
     WA(ievt,337) = b3 * b3 * b3 * x2 * weightscale;
     WA(ievt,338) = b3 * b3 * b3 * a2 * weightscale;
     WA(ievt,339) = b3 * b3 * b3 * y2 * weightscale;
     WA(ievt,340) = b3 * b3 * b3 * b2 * weightscale;

     WA(ievt,341) = x0 * x0 * x0 * t * weightscale;
     WA(ievt,342) = x0 * x0 * x0 * x3 * weightscale;
     WA(ievt,343) = x0 * x0 * x0 * a3 * weightscale;
     WA(ievt,344) = x0 * x0 * x0 * y3 * weightscale;
     WA(ievt,345) = x0 * x0 * x0 * b3 * weightscale;
     WA(ievt,346) = x0 * x0 * x0 * x2 * weightscale;
     WA(ievt,347) = x0 * x0 * x0 * a2 * weightscale;
     WA(ievt,348) = x0 * x0 * x0 * y2 * weightscale;
     WA(ievt,349) = x0 * x0 * x0 * b2 * weightscale;

     WA(ievt,350) = x2 * x2 * x2 * t * weightscale;
     WA(ievt,351) = x2 * x2 * x2 * x3 * weightscale;
     WA(ievt,352) = x2 * x2 * x2 * a3 * weightscale;
     WA(ievt,353) = x2 * x2 * x2 * y3 * weightscale;
     WA(ievt,354) = x2 * x2 * x2 * b3 * weightscale;
     WA(ievt,355) = x2 * x2 * x2 * x0 * weightscale;
     WA(ievt,356) = x2 * x2 * x2 * a2 * weightscale;
     WA(ievt,357) = x2 * x2 * x2 * y2 * weightscale;
     WA(ievt,358) = x2 * x2 * x2 * b2 * weightscale;

     WA(ievt,359) = a2 * a2 * a2 * t * weightscale;
     WA(ievt,360) = a2 * a2 * a2 * x3 * weightscale;
     WA(ievt,361) = a2 * a2 * a2 * a3 * weightscale;
     WA(ievt,362) = a2 * a2 * a2 * y3 * weightscale;
     WA(ievt,363) = a2 * a2 * a2 * b3 * weightscale;
     WA(ievt,364) = a2 * a2 * a2 * x0 * weightscale;
     WA(ievt,365) = a2 * a2 * a2 * x2 * weightscale;
     WA(ievt,366) = a2 * a2 * a2 * y2 * weightscale;
     WA(ievt,367) = a2 * a2 * a2 * b2 * weightscale;

     WA(ievt,368) = y2 * y2 * y2 * t * weightscale;
     WA(ievt,369) = y2 * y2 * y2 * x3 * weightscale;
     WA(ievt,370) = y2 * y2 * y2 * a3 * weightscale;
     WA(ievt,371) = y2 * y2 * y2 * y3 * weightscale;
     WA(ievt,372) = y2 * y2 * y2 * b3 * weightscale;
     WA(ievt,373) = y2 * y2 * y2 * x0 * weightscale;
     WA(ievt,374) = y2 * y2 * y2 * x2 * weightscale;
     WA(ievt,375) = y2 * y2 * y2 * a2 * weightscale;
     WA(ievt,376) = y2 * y2 * y2 * b2 * weightscale;

     WA(ievt,377) = b2 * b2 * b2 * t * weightscale;
     WA(ievt,378) = b2 * b2 * b2 * x3 * weightscale;
     WA(ievt,379) = b2 * b2 * b2 * a3 * weightscale;
     WA(ievt,380) = b2 * b2 * b2 * y3 * weightscale;
     WA(ievt,381) = b2 * b2 * b2 * b3 * weightscale;
     WA(ievt,382) = b2 * b2 * b2 * x0 * weightscale;
     WA(ievt,383) = b2 * b2 * b2 * x2 * weightscale;
     WA(ievt,384) = b2 * b2 * b2 * a2 * weightscale;
     WA(ievt,385) = b2 * b2 * b2 * y2 * weightscale;

     WA(ievt,386) = t * t * x3 * x3 * weightscale;
     WA(ievt,387) = t * t * a3 * a3 * weightscale;
     WA(ievt,388) = t * t * y3 * y3 * weightscale;
     WA(ievt,389) = t * t * b3 * b3 * weightscale;
     WA(ievt,390) = t * t * x0 * x0 * weightscale;
     WA(ievt,391) = t * t * x2 * x2 * weightscale;
     WA(ievt,392) = t * t * a2 * a2 * weightscale;
     WA(ievt,393) = t * t * y2 * y2 * weightscale;
     WA(ievt,394) = t * t * b2 * b2 * weightscale;

     WA(ievt,395) = x3 * x3 * a3 * a3 * weightscale;
     WA(ievt,396) = x3 * x3 * y3 * y3 * weightscale;
     WA(ievt,397) = x3 * x3 * b3 * b3 * weightscale;
     WA(ievt,398) = x3 * x3 * x0 * x0 * weightscale;
     WA(ievt,399) = x3 * x3 * x2 * x2 * weightscale;
     WA(ievt,400) = x3 * x3 * a2 * a2 * weightscale;
     WA(ievt,401) = x3 * x3 * y2 * y2 * weightscale;
     WA(ievt,402) = x3 * x3 * b2 * b2 * weightscale;

     WA(ievt,403) = a3 * a3 * y3 * y3 * weightscale;
     WA(ievt,404) = a3 * a3 * b3 * b3 * weightscale;
     WA(ievt,405) = a3 * a3 * x0 * x0 * weightscale;
     WA(ievt,406) = a3 * a3 * x2 * x2 * weightscale;
     WA(ievt,407) = a3 * a3 * a2 * a2 * weightscale;
     WA(ievt,408) = a3 * a3 * y2 * y2 * weightscale;
     WA(ievt,409) = a3 * a3 * b2 * b2 * weightscale;

     WA(ievt,410) = y3 * y3 * b3 * b3 * weightscale;
     WA(ievt,411) = y3 * y3 * x0 * x0 * weightscale;
     WA(ievt,412) = y3 * y3 * x2 * x2 * weightscale;
     WA(ievt,413) = y3 * y3 * a2 * a2 * weightscale;
     WA(ievt,414) = y3 * y3 * y2 * y2 * weightscale;
     WA(ievt,415) = y3 * y3 * b2 * b2 * weightscale;

     WA(ievt,416) = b3 * b3 * x0 * x0 * weightscale;
     WA(ievt,417) = b3 * b3 * x2 * x2 * weightscale;
     WA(ievt,418) = b3 * b3 * a2 * a2 * weightscale;
     WA(ievt,419) = b3 * b3 * y2 * y2 * weightscale;
     WA(ievt,420) = b3 * b3 * b2 * b2 * weightscale;

     WA(ievt,421) = x0 * x0 * x2 * x2 * weightscale;
     WA(ievt,422) = x0 * x0 * a2 * a2 * weightscale;
     WA(ievt,423) = x0 * x0 * y2 * y2 * weightscale;
     WA(ievt,424) = x0 * x0 * b2 * b2 * weightscale;

     WA(ievt,425) = x2 * x2 * a2 * a2 * weightscale;
     WA(ievt,426) = x2 * x2 * y2 * y2 * weightscale;
     WA(ievt,427) = x2 * x2 * b2 * b2 * weightscale;

     WA(ievt,428) = a2 * a2 * y2 * y2 * weightscale;
     WA(ievt,429) = a2 * a2 * b2 * b2 * weightscale;

     WA(ievt,430) = y2 * y2 * b2 * b2 * weightscale;

     WA(ievt,431) = t * t * x3 * a3 * weightscale;
     WA(ievt,432) = t * t * x3 * y3 * weightscale;
     WA(ievt,433) = t * t * x3 * b3 * weightscale;
     WA(ievt,434) = t * t * x3 * x0 * weightscale;
     WA(ievt,435) = t * t * x3 * x2 * weightscale;
     WA(ievt,436) = t * t * x3 * a2 * weightscale;
     WA(ievt,437) = t * t * x3 * y2 * weightscale;
     WA(ievt,438) = t * t * x3 * b2 * weightscale;

     WA(ievt,439) = t * t * a3 * y3 * weightscale;
     WA(ievt,440) = t * t * a3 * b3 * weightscale;
     WA(ievt,441) = t * t * a3 * x0 * weightscale;
     WA(ievt,442) = t * t * a3 * x2 * weightscale;
     WA(ievt,443) = t * t * a3 * a2 * weightscale;
     WA(ievt,444) = t * t * a3 * y2 * weightscale;
     WA(ievt,445) = t * t * a3 * b2 * weightscale;

     WA(ievt,446) = t * t * y3 * b3 * weightscale;
     WA(ievt,447) = t * t * y3 * x0 * weightscale;
     WA(ievt,448) = t * t * y3 * x2 * weightscale;
     WA(ievt,449) = t * t * y3 * a2 * weightscale;
     WA(ievt,450) = t * t * y3 * y2 * weightscale;
     WA(ievt,451) = t * t * y3 * b2 * weightscale;

     WA(ievt,452) = t * t * b3 * x0 * weightscale;
     WA(ievt,453) = t * t * b3 * x2 * weightscale;
     WA(ievt,454) = t * t * b3 * a2 * weightscale;
     WA(ievt,455) = t * t * b3 * y2 * weightscale;
     WA(ievt,456) = t * t * b3 * b2 * weightscale;

     WA(ievt,457) = t * t * x0 * x2 * weightscale;
     WA(ievt,458) = t * t * x0 * a2 * weightscale;
     WA(ievt,459) = t * t * x0 * y2 * weightscale;
     WA(ievt,460) = t * t * x0 * b2 * weightscale;

     WA(ievt,461) = t * t * x2 * a2 * weightscale;
     WA(ievt,462) = t * t * x2 * y2 * weightscale;
     WA(ievt,463) = t * t * x2 * b2 * weightscale;

     WA(ievt,464) = t * t * a2 * y2 * weightscale;
     WA(ievt,465) = t * t * a2 * b2 * weightscale;

     WA(ievt,466) = t * t * y2 * b2 * weightscale;

     WA(ievt,467) = x3 * x3 * t * a3 * weightscale;
     WA(ievt,468) = x3 * x3 * t * y3 * weightscale;
     WA(ievt,469) = x3 * x3 * t * b3 * weightscale;
     WA(ievt,470) = x3 * x3 * t * x0 * weightscale;
     WA(ievt,471) = x3 * x3 * t * x2 * weightscale;
     WA(ievt,472) = x3 * x3 * t * a2 * weightscale;
     WA(ievt,473) = x3 * x3 * t * y2 * weightscale;
     WA(ievt,474) = x3 * x3 * t * b2 * weightscale;

     WA(ievt,475) = x3 * x3 * a3 * y3 * weightscale;
     WA(ievt,476) = x3 * x3 * a3 * b3 * weightscale;
     WA(ievt,477) = x3 * x3 * a3 * x0 * weightscale;
     WA(ievt,478) = x3 * x3 * a3 * x2 * weightscale;
     WA(ievt,479) = x3 * x3 * a3 * a2 * weightscale;
     WA(ievt,480) = x3 * x3 * a3 * y2 * weightscale;
     WA(ievt,481) = x3 * x3 * a3 * b2 * weightscale;

     WA(ievt,482) = x3 * x3 * y3 * b3 * weightscale;
     WA(ievt,483) = x3 * x3 * y3 * x0 * weightscale;
     WA(ievt,484) = x3 * x3 * y3 * x2 * weightscale;
     WA(ievt,485) = x3 * x3 * y3 * a2 * weightscale;
     WA(ievt,486) = x3 * x3 * y3 * y2 * weightscale;
     WA(ievt,487) = x3 * x3 * y3 * b2 * weightscale;

     WA(ievt,488) = x3 * x3 * b3 * x0 * weightscale;
     WA(ievt,489) = x3 * x3 * b3 * x2 * weightscale;
     WA(ievt,490) = x3 * x3 * b3 * a2 * weightscale;
     WA(ievt,491) = x3 * x3 * b3 * y2 * weightscale;
     WA(ievt,492) = x3 * x3 * b3 * b2 * weightscale;

     WA(ievt,493) = x3 * x3 * x0 * x2 * weightscale;
     WA(ievt,494) = x3 * x3 * x0 * a2 * weightscale;
     WA(ievt,495) = x3 * x3 * x0 * y2 * weightscale;
     WA(ievt,496) = x3 * x3 * x0 * b2 * weightscale;

     WA(ievt,497) = x3 * x3 * x2 * a2 * weightscale;
     WA(ievt,498) = x3 * x3 * x2 * y2 * weightscale;
     WA(ievt,499) = x3 * x3 * x2 * b2 * weightscale;

     WA(ievt,500) = x3 * x3 * a2 * y2 * weightscale;
     WA(ievt,501) = x3 * x3 * a2 * b2 * weightscale;

     WA(ievt,502) = x3 * x3 * y2 * b2 * weightscale;


     WA(ievt,503) = a3 * a3 * t * x3 * weightscale;
     WA(ievt,504) = a3 * a3 * t * y3 * weightscale;
     WA(ievt,505) = a3 * a3 * t * b3 * weightscale;
     WA(ievt,506) = a3 * a3 * t * x0 * weightscale;
     WA(ievt,507) = a3 * a3 * t * x2 * weightscale;
     WA(ievt,508) = a3 * a3 * t * a2 * weightscale;
     WA(ievt,509) = a3 * a3 * t * y2 * weightscale;
     WA(ievt,510) = a3 * a3 * t * b2 * weightscale;

     WA(ievt,511) = a3 * a3 * x3 * y3 * weightscale;
     WA(ievt,512) = a3 * a3 * x3 * b3 * weightscale;
     WA(ievt,513) = a3 * a3 * x3 * x0 * weightscale;
     WA(ievt,514) = a3 * a3 * x3 * x2 * weightscale;
     WA(ievt,515) = a3 * a3 * x3 * a2 * weightscale;
     WA(ievt,516) = a3 * a3 * x3 * y2 * weightscale;
     WA(ievt,517) = a3 * a3 * x3 * b2 * weightscale;

     WA(ievt,518) = a3 * a3 * y3 * b3 * weightscale;
     WA(ievt,519) = a3 * a3 * y3 * x0 * weightscale;
     WA(ievt,520) = a3 * a3 * y3 * x2 * weightscale;
     WA(ievt,521) = a3 * a3 * y3 * a2 * weightscale;
     WA(ievt,522) = a3 * a3 * y3 * y2 * weightscale;
     WA(ievt,523) = a3 * a3 * y3 * b2 * weightscale;

     WA(ievt,524) = a3 * a3 * b3 * x0 * weightscale;
     WA(ievt,525) = a3 * a3 * b3 * x2 * weightscale;
     WA(ievt,526) = a3 * a3 * b3 * a2 * weightscale;
     WA(ievt,527) = a3 * a3 * b3 * y2 * weightscale;
     WA(ievt,528) = a3 * a3 * b3 * b2 * weightscale;

     WA(ievt,529) = a3 * a3 * x0 * x2 * weightscale;
     WA(ievt,530) = a3 * a3 * x0 * a2 * weightscale;
     WA(ievt,531) = a3 * a3 * x0 * y2 * weightscale;
     WA(ievt,532) = a3 * a3 * x0 * b2 * weightscale;

     WA(ievt,533) = a3 * a3 * x2 * a2 * weightscale;
     WA(ievt,534) = a3 * a3 * x2 * y2 * weightscale;
     WA(ievt,535) = a3 * a3 * x2 * b2 * weightscale;

     WA(ievt,536) = a3 * a3 * a2 * y2 * weightscale;
     WA(ievt,537) = a3 * a3 * a2 * b2 * weightscale;

     WA(ievt,538) = a3 * a3 * y2 * b2 * weightscale;

     WA(ievt,539) = y3 * y3 * t * x3 * weightscale;
     WA(ievt,540) = y3 * y3 * t * a3 * weightscale;
     WA(ievt,541) = y3 * y3 * t * b3 * weightscale;
     WA(ievt,542) = y3 * y3 * t * x0 * weightscale;
     WA(ievt,543) = y3 * y3 * t * x2 * weightscale;
     WA(ievt,544) = y3 * y3 * t * a2 * weightscale;
     WA(ievt,545) = y3 * y3 * t * y2 * weightscale;
     WA(ievt,546) = y3 * y3 * t * b2 * weightscale;

     WA(ievt,547) = y3 * y3 * x3 * a3 * weightscale;
     WA(ievt,548) = y3 * y3 * x3 * b3 * weightscale;
     WA(ievt,549) = y3 * y3 * x3 * x0 * weightscale;
     WA(ievt,550) = y3 * y3 * x3 * x2 * weightscale;
     WA(ievt,551) = y3 * y3 * x3 * a2 * weightscale;
     WA(ievt,552) = y3 * y3 * x3 * y2 * weightscale;
     WA(ievt,553) = y3 * y3 * x3 * b2 * weightscale;

     WA(ievt,554) = y3 * y3 * a3 * b3 * weightscale;
     WA(ievt,555) = y3 * y3 * a3 * x0 * weightscale;
     WA(ievt,556) = y3 * y3 * a3 * x2 * weightscale;
     WA(ievt,557) = y3 * y3 * a3 * a2 * weightscale;
     WA(ievt,558) = y3 * y3 * a3 * y2 * weightscale;
     WA(ievt,559) = y3 * y3 * a3 * b2 * weightscale;

     WA(ievt,560) = y3 * y3 * b3 * x0 * weightscale;
     WA(ievt,561) = y3 * y3 * b3 * x2 * weightscale;
     WA(ievt,562) = y3 * y3 * b3 * a2 * weightscale;
     WA(ievt,563) = y3 * y3 * b3 * y2 * weightscale;
     WA(ievt,564) = y3 * y3 * b3 * b2 * weightscale;

     WA(ievt,565) = y3 * y3 * x0 * x2 * weightscale;
     WA(ievt,566) = y3 * y3 * x0 * a2 * weightscale;
     WA(ievt,567) = y3 * y3 * x0 * y2 * weightscale;
     WA(ievt,568) = y3 * y3 * x0 * b2 * weightscale;

     WA(ievt,569) = y3 * y3 * x2 * a2 * weightscale;
     WA(ievt,570) = y3 * y3 * x2 * y2 * weightscale;
     WA(ievt,571) = y3 * y3 * x2 * b2 * weightscale;

     WA(ievt,572) = y3 * y3 * a2 * y2 * weightscale;
     WA(ievt,573) = y3 * y3 * a2 * b2 * weightscale;

     WA(ievt,574) = y3 * y3 * y2 * b2 * weightscale;

     WA(ievt,575) = b3 * b3 * t * x3 * weightscale;
     WA(ievt,576) = b3 * b3 * t * a3 * weightscale;
     WA(ievt,577) = b3 * b3 * t * y3 * weightscale;
     WA(ievt,578) = b3 * b3 * t * x0 * weightscale;
     WA(ievt,579) = b3 * b3 * t * x2 * weightscale;
     WA(ievt,580) = b3 * b3 * t * a2 * weightscale;
     WA(ievt,581) = b3 * b3 * t * y2 * weightscale;
     WA(ievt,582) = b3 * b3 * t * b2 * weightscale;

     WA(ievt,583) = b3 * b3 * x3 * a3 * weightscale;
     WA(ievt,584) = b3 * b3 * x3 * y3 * weightscale;
     WA(ievt,585) = b3 * b3 * x3 * x0 * weightscale;
     WA(ievt,586) = b3 * b3 * x3 * x2 * weightscale;
     WA(ievt,587) = b3 * b3 * x3 * a2 * weightscale;
     WA(ievt,588) = b3 * b3 * x3 * y2 * weightscale;
     WA(ievt,589) = b3 * b3 * x3 * b2 * weightscale;

     WA(ievt,590) = b3 * b3 * a3 * y3 * weightscale;
     WA(ievt,591) = b3 * b3 * a3 * x0 * weightscale;
     WA(ievt,592) = b3 * b3 * a3 * x2 * weightscale;
     WA(ievt,593) = b3 * b3 * a3 * a2 * weightscale;
     WA(ievt,594) = b3 * b3 * a3 * y2 * weightscale;
     WA(ievt,595) = b3 * b3 * a3 * b2 * weightscale;

     WA(ievt,596) = b3 * b3 * y3 * x0 * weightscale;
     WA(ievt,597) = b3 * b3 * y3 * x2 * weightscale;
     WA(ievt,598) = b3 * b3 * y3 * a2 * weightscale;
     WA(ievt,599) = b3 * b3 * y3 * y2 * weightscale;
     WA(ievt,600) = b3 * b3 * y3 * b2 * weightscale;

     WA(ievt,601) = b3 * b3 * x0 * x2 * weightscale;
     WA(ievt,602) = b3 * b3 * x0 * a2 * weightscale;
     WA(ievt,603) = b3 * b3 * x0 * y2 * weightscale;
     WA(ievt,604) = b3 * b3 * x0 * b2 * weightscale;

     WA(ievt,605) = b3 * b3 * x2 * a2 * weightscale;
     WA(ievt,606) = b3 * b3 * x2 * y2 * weightscale;
     WA(ievt,607) = b3 * b3 * x2 * b2 * weightscale;

     WA(ievt,608) = b3 * b3 * a2 * y2 * weightscale;
     WA(ievt,609) = b3 * b3 * a2 * b2 * weightscale;

     WA(ievt,610) = b3 * b3 * y2 * b2 * weightscale;

     WA(ievt,611) = x0 * x0 * t * x3 * weightscale;
     WA(ievt,612) = x0 * x0 * t * a3 * weightscale;
     WA(ievt,613) = x0 * x0 * t * y3 * weightscale;
     WA(ievt,614) = x0 * x0 * t * b3 * weightscale;
     WA(ievt,615) = x0 * x0 * t * x2 * weightscale;
     WA(ievt,616) = x0 * x0 * t * a2 * weightscale;
     WA(ievt,617) = x0 * x0 * t * y2 * weightscale;
     WA(ievt,618) = x0 * x0 * t * b2 * weightscale;

     WA(ievt,619) = x0 * x0 * x3 * a3 * weightscale;
     WA(ievt,620) = x0 * x0 * x3 * y3 * weightscale;
     WA(ievt,621) = x0 * x0 * x3 * b3 * weightscale;
     WA(ievt,622) = x0 * x0 * x3 * x2 * weightscale;
     WA(ievt,623) = x0 * x0 * x3 * a2 * weightscale;
     WA(ievt,624) = x0 * x0 * x3 * y2 * weightscale;
     WA(ievt,625) = x0 * x0 * x3 * b2 * weightscale;

     WA(ievt,626) = x0 * x0 * a3 * y3 * weightscale;
     WA(ievt,627) = x0 * x0 * a3 * b3 * weightscale;
     WA(ievt,628) = x0 * x0 * a3 * x2 * weightscale;
     WA(ievt,629) = x0 * x0 * a3 * a2 * weightscale;
     WA(ievt,630) = x0 * x0 * a3 * y2 * weightscale;
     WA(ievt,631) = x0 * x0 * a3 * b2 * weightscale;

     WA(ievt,632) = x0 * x0 * y3 * b3 * weightscale;
     WA(ievt,633) = x0 * x0 * y3 * x2 * weightscale;
     WA(ievt,634) = x0 * x0 * y3 * a2 * weightscale;
     WA(ievt,635) = x0 * x0 * y3 * y2 * weightscale;
     WA(ievt,636) = x0 * x0 * y3 * b2 * weightscale;

     WA(ievt,637) = x0 * x0 * b3 * x2 * weightscale;
     WA(ievt,638) = x0 * x0 * b3 * a2 * weightscale;
     WA(ievt,639) = x0 * x0 * b3 * y2 * weightscale;
     WA(ievt,640) = x0 * x0 * b3 * b2 * weightscale;

     WA(ievt,641) = x0 * x0 * x2 * a2 * weightscale;
     WA(ievt,642) = x0 * x0 * x2 * y2 * weightscale;
     WA(ievt,643) = x0 * x0 * x2 * b2 * weightscale;

     WA(ievt,644) = x0 * x0 * a2 * y2 * weightscale;
     WA(ievt,645) = x0 * x0 * a2 * b2 * weightscale;

     WA(ievt,646) = x0 * x0 * y2 * b2 * weightscale;

     WA(ievt,647) = x2 * x2 * t * x3 * weightscale;
     WA(ievt,648) = x2 * x2 * t * a3 * weightscale;
     WA(ievt,649) = x2 * x2 * t * y3 * weightscale;
     WA(ievt,650) = x2 * x2 * t * b3 * weightscale;
     WA(ievt,651) = x2 * x2 * t * x0 * weightscale;
     WA(ievt,652) = x2 * x2 * t * a2 * weightscale;
     WA(ievt,653) = x2 * x2 * t * y2 * weightscale;
     WA(ievt,654) = x2 * x2 * t * b2 * weightscale;

     WA(ievt,655) = x2 * x2 * x3 * a3 * weightscale;
     WA(ievt,656) = x2 * x2 * x3 * y3 * weightscale;
     WA(ievt,657) = x2 * x2 * x3 * b3 * weightscale;
     WA(ievt,658) = x2 * x2 * x3 * x0 * weightscale;
     WA(ievt,659) = x2 * x2 * x3 * a2 * weightscale;
     WA(ievt,660) = x2 * x2 * x3 * y2 * weightscale;
     WA(ievt,661) = x2 * x2 * x3 * b2 * weightscale;

     WA(ievt,662) = x2 * x2 * a3 * y3 * weightscale;
     WA(ievt,663) = x2 * x2 * a3 * b3 * weightscale;
     WA(ievt,664) = x2 * x2 * a3 * x0 * weightscale;
     WA(ievt,665) = x2 * x2 * a3 * a2 * weightscale;
     WA(ievt,666) = x2 * x2 * a3 * y2 * weightscale;
     WA(ievt,667) = x2 * x2 * a3 * b2 * weightscale;

     WA(ievt,668) = x2 * x2 * y3 * b3 * weightscale;
     WA(ievt,669) = x2 * x2 * y3 * x0 * weightscale;
     WA(ievt,670) = x2 * x2 * y3 * a2 * weightscale;
     WA(ievt,671) = x2 * x2 * y3 * y2 * weightscale;
     WA(ievt,672) = x2 * x2 * y3 * b2 * weightscale;

     WA(ievt,673) = x2 * x2 * b3 * x0 * weightscale;
     WA(ievt,674) = x2 * x2 * b3 * a2 * weightscale;
     WA(ievt,675) = x2 * x2 * b3 * y2 * weightscale;
     WA(ievt,676) = x2 * x2 * b3 * b2 * weightscale;

     WA(ievt,677) = x2 * x2 * x0 * a2 * weightscale;
     WA(ievt,678) = x2 * x2 * x0 * y2 * weightscale;
     WA(ievt,679) = x2 * x2 * x0 * b2 * weightscale;

     WA(ievt,680) = x2 * x2 * a2 * y2 * weightscale;
     WA(ievt,681) = x2 * x2 * a2 * b2 * weightscale;

     WA(ievt,682) = x2 * x2 * y2 * b2 * weightscale;

     WA(ievt,683) = a2 * a2 * t * x3 * weightscale;
     WA(ievt,684) = a2 * a2 * t * a3 * weightscale;
     WA(ievt,685) = a2 * a2 * t * y3 * weightscale;
     WA(ievt,686) = a2 * a2 * t * b3 * weightscale;
     WA(ievt,687) = a2 * a2 * t * x0 * weightscale;
     WA(ievt,688) = a2 * a2 * t * x2 * weightscale;
     WA(ievt,689) = a2 * a2 * t * y2 * weightscale;
     WA(ievt,691) = a2 * a2 * t * b2 * weightscale;

     WA(ievt,692) = a2 * a2 * x3 * a3 * weightscale;
     WA(ievt,694) = a2 * a2 * x3 * y3 * weightscale;
     WA(ievt,695) = a2 * a2 * x3 * b3 * weightscale;
     WA(ievt,696) = a2 * a2 * x3 * x0 * weightscale;
     WA(ievt,697) = a2 * a2 * x3 * x2 * weightscale;
     WA(ievt,698) = a2 * a2 * x3 * y2 * weightscale;
     WA(ievt,699) = a2 * a2 * x3 * b2 * weightscale;

     WA(ievt,700) = a2 * a2 * a3 * y3 * weightscale;
     WA(ievt,701) = a2 * a2 * a3 * b3 * weightscale;
     WA(ievt,702) = a2 * a2 * a3 * x0 * weightscale;
     WA(ievt,703) = a2 * a2 * a3 * x2 * weightscale;
     WA(ievt,704) = a2 * a2 * a3 * y2 * weightscale;
     WA(ievt,705) = a2 * a2 * a3 * b2 * weightscale;

     WA(ievt,706) = a2 * a2 * y3 * b3 * weightscale;
     WA(ievt,707) = a2 * a2 * y3 * x0 * weightscale;
     WA(ievt,708) = a2 * a2 * y3 * x2 * weightscale;
     WA(ievt,709) = a2 * a2 * y3 * y2 * weightscale;
     WA(ievt,710) = a2 * a2 * y3 * b2 * weightscale;

     WA(ievt,712) = a2 * a2 * b3 * x0 * weightscale;
     WA(ievt,713) = a2 * a2 * b3 * x2 * weightscale;
     WA(ievt,714) = a2 * a2 * b3 * y2 * weightscale;
     WA(ievt,715) = a2 * a2 * b3 * b2 * weightscale;

     WA(ievt,716) = a2 * a2 * x0 * x2 * weightscale;
     WA(ievt,717) = a2 * a2 * x0 * y2 * weightscale;
     WA(ievt,718) = a2 * a2 * x0 * b2 * weightscale;

     WA(ievt,719) = a2 * a2 * x2 * y2 * weightscale;
     WA(ievt,720) = a2 * a2 * x2 * b2 * weightscale;

     WA(ievt,721) = a2 * a2 * y2 * b2 * weightscale;

     WA(ievt,722) = y2 * y2 * t * x3 * weightscale;
     WA(ievt,723) = y2 * y2 * t * a3 * weightscale;
     WA(ievt,724) = y2 * y2 * t * y3 * weightscale;
     WA(ievt,725) = y2 * y2 * t * b3 * weightscale;
     WA(ievt,726) = y2 * y2 * t * x0 * weightscale;
     WA(ievt,727) = y2 * y2 * t * x2 * weightscale;
     WA(ievt,728) = y2 * y2 * t * a2 * weightscale;
     WA(ievt,729) = y2 * y2 * t * b2 * weightscale;

     WA(ievt,730) = y2 * y2 * x3 * a3 * weightscale;
     WA(ievt,731) = y2 * y2 * x3 * y3 * weightscale;
     WA(ievt,732) = y2 * y2 * x3 * b3 * weightscale;
     WA(ievt,733) = y2 * y2 * x3 * x0 * weightscale;
     WA(ievt,734) = y2 * y2 * x3 * x2 * weightscale;
     WA(ievt,735) = y2 * y2 * x3 * a2 * weightscale;
     WA(ievt,736) = y2 * y2 * x3 * b2 * weightscale;

     WA(ievt,737) = y2 * y2 * a3 * y3 * weightscale;
     WA(ievt,734) = y2 * y2 * a3 * b3 * weightscale;
     WA(ievt,735) = y2 * y2 * a3 * x0 * weightscale;
     WA(ievt,736) = y2 * y2 * a3 * x2 * weightscale;
     WA(ievt,737) = y2 * y2 * a3 * a2 * weightscale;
     WA(ievt,738) = y2 * y2 * a3 * b2 * weightscale;

     WA(ievt,739) = y2 * y2 * y3 * b3 * weightscale;
     WA(ievt,740) = y2 * y2 * y3 * x0 * weightscale;
     WA(ievt,741) = y2 * y2 * y3 * x2 * weightscale;
     WA(ievt,742) = y2 * y2 * y3 * a2 * weightscale;
     WA(ievt,743) = y2 * y2 * y3 * b2 * weightscale;

     WA(ievt,744) = y2 * y2 * b3 * x0 * weightscale;
     WA(ievt,745) = y2 * y2 * b3 * x2 * weightscale;
     WA(ievt,746) = y2 * y2 * b3 * a2 * weightscale;
     WA(ievt,747) = y2 * y2 * b3 * b2 * weightscale;

     WA(ievt,748) = y2 * y2 * x0 * x2 * weightscale;
     WA(ievt,749) = y2 * y2 * x0 * a2 * weightscale;
     WA(ievt,750) = y2 * y2 * x0 * b2 * weightscale;

     WA(ievt,751) = y2 * y2 * x2 * a2 * weightscale;
     WA(ievt,752) = y2 * y2 * x2 * b2 * weightscale;

     WA(ievt,753) = y2 * y2 * a2 * b2 * weightscale;

     WA(ievt,754) = b2 * b2 * t * x3 * weightscale;
     WA(ievt,755) = b2 * b2 * t * a3 * weightscale;
     WA(ievt,756) = b2 * b2 * t * y3 * weightscale;
     WA(ievt,757) = b2 * b2 * t * b3 * weightscale;
     WA(ievt,758) = b2 * b2 * t * x0 * weightscale;
     WA(ievt,759) = b2 * b2 * t * x2 * weightscale;
     WA(ievt,760) = b2 * b2 * t * a2 * weightscale;
     WA(ievt,761) = b2 * b2 * t * y2 * weightscale;

     WA(ievt,762) = b2 * b2 * x3 * a3 * weightscale;
     WA(ievt,763) = b2 * b2 * x3 * y3 * weightscale;
     WA(ievt,764) = b2 * b2 * x3 * b3 * weightscale;
     WA(ievt,765) = b2 * b2 * x3 * x0 * weightscale;
     WA(ievt,766) = b2 * b2 * x3 * x2 * weightscale;
     WA(ievt,767) = b2 * b2 * x3 * a2 * weightscale;
     WA(ievt,768) = b2 * b2 * x3 * y2 * weightscale;

     WA(ievt,769) = b2 * b2 * a3 * y3 * weightscale;
     WA(ievt,770) = b2 * b2 * a3 * b3 * weightscale;
     WA(ievt,771) = b2 * b2 * a3 * x0 * weightscale;
     WA(ievt,772) = b2 * b2 * a3 * x2 * weightscale;
     WA(ievt,773) = b2 * b2 * a3 * a2 * weightscale;
     WA(ievt,774) = b2 * b2 * a3 * y2 * weightscale;

     WA(ievt,775) = b2 * b2 * y3 * b3 * weightscale;
     WA(ievt,776) = b2 * b2 * y3 * x0 * weightscale;
     WA(ievt,778) = b2 * b2 * y3 * x2 * weightscale;
     WA(ievt,779) = b2 * b2 * y3 * a2 * weightscale;
     WA(ievt,780) = b2 * b2 * y3 * y2 * weightscale;

     WA(ievt,781) = b2 * b2 * b3 * x0 * weightscale;
     WA(ievt,782) = b2 * b2 * b3 * x2 * weightscale;
     WA(ievt,783) = b2 * b2 * b3 * a2 * weightscale;
     WA(ievt,784) = b2 * b2 * b3 * y2 * weightscale;

     WA(ievt,785) = b2 * b2 * x0 * x2 * weightscale;
     WA(ievt,786) = b2 * b2 * x0 * a2 * weightscale;
     WA(ievt,787) = b2 * b2 * x0 * y2 * weightscale;

     WA(ievt,788) = b2 * b2 * x2 * a2 * weightscale;
     WA(ievt,789) = b2 * b2 * x2 * y2 * weightscale;

     WA(ievt,790) = b2 * b2 * a2 * y2 * weightscale;

     WA(ievt,791) = t * x3 * a3 * y3 * weightscale;
     WA(ievt,792) = t * x3 * a3 * b3 * weightscale;
     WA(ievt,793) = t * x3 * a3 * x0 * weightscale;
     WA(ievt,794) = t * x3 * a3 * x2 * weightscale;
     WA(ievt,795) = t * x3 * a3 * a2 * weightscale;
     WA(ievt,796) = t * x3 * a3 * y2 * weightscale;
     WA(ievt,797) = t * x3 * a3 * b2 * weightscale;

     WA(ievt,798) = t * x3 * y3 * b3 * weightscale;
     WA(ievt,799) = t * x3 * y3 * x0 * weightscale;
     WA(ievt,800) = t * x3 * y3 * x2 * weightscale;
     WA(ievt,801) = t * x3 * y3 * a2 * weightscale;
     WA(ievt,802) = t * x3 * y3 * y2 * weightscale;
     WA(ievt,803) = t * x3 * y3 * b2 * weightscale;

     WA(ievt,804) = t * x3 * b3 * x0 * weightscale;
     WA(ievt,805) = t * x3 * b3 * x2 * weightscale;
     WA(ievt,806) = t * x3 * b3 * a2 * weightscale;
     WA(ievt,807) = t * x3 * b3 * y2 * weightscale;
     WA(ievt,808) = t * x3 * b3 * b2 * weightscale;

     WA(ievt,809) = t * x3 * x0 * x2 * weightscale;
     WA(ievt,810) = t * x3 * x0 * a2 * weightscale;
     WA(ievt,811) = t * x3 * x0 * y2 * weightscale;
     WA(ievt,812) = t * x3 * x0 * b2 * weightscale;

     WA(ievt,813) = t * x3 * x2 * a2 * weightscale;
     WA(ievt,814) = t * x3 * x2 * y2 * weightscale;
     WA(ievt,815) = t * x3 * x2 * b2 * weightscale;
     WA(ievt,816) = t * x3 * a2 * y2 * weightscale;
     WA(ievt,817) = t * x3 * a2 * b2 * weightscale;

     WA(ievt,818) = t * x3 * y2 * b2 * weightscale;

     WA(ievt,819) = t * a3 * y3 * b3 * weightscale;
     WA(ievt,820) = t * a3 * y3 * x0 * weightscale;
     WA(ievt,821) = t * a3 * y3 * x2 * weightscale;
     WA(ievt,822) = t * a3 * y3 * a2 * weightscale;
     WA(ievt,823) = t * a3 * y3 * y2 * weightscale;
     WA(ievt,824) = t * a3 * y3 * b2 * weightscale;

     WA(ievt,825) = t * a3 * b3 * x0 * weightscale;
     WA(ievt,826) = t * a3 * b3 * x2 * weightscale;
     WA(ievt,827) = t * a3 * b3 * a2 * weightscale;
     WA(ievt,828) = t * a3 * b3 * y2 * weightscale;
     WA(ievt,829) = t * a3 * b3 * b2 * weightscale;

     WA(ievt,830) = t * a3 * x0 * x2 * weightscale;
     WA(ievt,831) = t * a3 * x0 * a2 * weightscale;
     WA(ievt,832) = t * a3 * x0 * y2 * weightscale;
     WA(ievt,833) = t * a3 * x0 * b2 * weightscale;

     WA(ievt,834) = t * a3 * x2 * a2 * weightscale;
     WA(ievt,835) = t * a3 * x2 * y2 * weightscale;
     WA(ievt,836) = t * a3 * x2 * b2 * weightscale;

     WA(ievt,837) = t * a3 * a2 * y2 * weightscale;
     WA(ievt,838) = t * a3 * a2 * b2 * weightscale;

     WA(ievt,839) = t * a3 * y2 * b2 * weightscale;

     WA(ievt,840) = t * y3 * b3 * x0 * weightscale;
     WA(ievt,841) = t * y3 * b3 * x2 * weightscale;
     WA(ievt,842) = t * y3 * b3 * a2 * weightscale;
     WA(ievt,843) = t * y3 * b3 * y2 * weightscale;
     WA(ievt,844) = t * y3 * b3 * b2 * weightscale;

     WA(ievt,845) = t * y3 * x0 * x2 * weightscale;
     WA(ievt,846) = t * y3 * x0 * a2 * weightscale;
     WA(ievt,847) = t * y3 * x0 * y2 * weightscale;
     WA(ievt,848) = t * y3 * x0 * b2 * weightscale;

     WA(ievt,849) = t * y3 * x2 * a2 * weightscale;
     WA(ievt,850) = t * y3 * x2 * y2 * weightscale;
     WA(ievt,851) = t * y3 * x2 * b2 * weightscale;

     WA(ievt,852) = t * y3 * a2 * y2 * weightscale;
     WA(ievt,853) = t * y3 * a2 * b2 * weightscale;

     WA(ievt,854) = t * y3 * y2 * b2 * weightscale;

     WA(ievt,855) = t * b3 * x0 * x2 * weightscale;
     WA(ievt,856) = t * b3 * x0 * a2 * weightscale;
     WA(ievt,857) = t * b3 * x0 * y2 * weightscale;
     WA(ievt,858) = t * b3 * x0 * b2 * weightscale;

     WA(ievt,859) = t * b3 * x2 * a2 * weightscale;
     WA(ievt,860) = t * b3 * x2 * y2 * weightscale;
     WA(ievt,861) = t * b3 * x2 * b2 * weightscale;

     WA(ievt,862) = t * b3 * a2 * y2 * weightscale;
     WA(ievt,863) = t * b3 * a2 * b2 * weightscale;

     WA(ievt,864) = t * b3 * y2 * b2 * weightscale;

     WA(ievt,865) = t * x0 * x2 * a2 * weightscale;
     WA(ievt,866) = t * x0 * x2 * y2 * weightscale;
     WA(ievt,867) = t * x0 * x2 * b2 * weightscale;

     WA(ievt,868) = t * x0 * a2 * y2 * weightscale;
     WA(ievt,869) = t * x0 * a2 * b2 * weightscale;

     WA(ievt,870) = t * x0 * y2 * b2 * weightscale;

     WA(ievt,871) = t * x2 * a2 * y2 * weightscale;
     WA(ievt,872) = t * x2 * a2 * b2 * weightscale;

     WA(ievt,873) = t * x2 * y2 * b2 * weightscale;

     WA(ievt,874) = t * a2 * y2 * b2 * weightscale;

     WA(ievt,875) = x3 * a3 * y3 * b3 * weightscale;
     WA(ievt,876) = x3 * a3 * y3 * x0 * weightscale;
     WA(ievt,877) = x3 * a3 * y3 * x2 * weightscale;
     WA(ievt,878) = x3 * a3 * y3 * a2 * weightscale;
     WA(ievt,879) = x3 * a3 * y3 * y2 * weightscale;
     WA(ievt,880) = x3 * a3 * y3 * b2 * weightscale;

     WA(ievt,881) = x3 * a3 * b3 * x0 * weightscale;
     WA(ievt,882) = x3 * a3 * b3 * x2 * weightscale;
     WA(ievt,883) = x3 * a3 * b3 * a2 * weightscale;
     WA(ievt,884) = x3 * a3 * b3 * y2 * weightscale;
     WA(ievt,885) = x3 * a3 * b3 * b2 * weightscale;

     WA(ievt,886) = x3 * a3 * x0 * x2 * weightscale;
     WA(ievt,887) = x3 * a3 * x0 * a2 * weightscale;
     WA(ievt,888) = x3 * a3 * x0 * y2 * weightscale;
     WA(ievt,889) = x3 * a3 * x0 * b2 * weightscale;

     WA(ievt,890) = x3 * a3 * x2 * a2 * weightscale;
     WA(ievt,891) = x3 * a3 * x2 * y2 * weightscale;
     WA(ievt,892) = x3 * a3 * x2 * b2 * weightscale;

     WA(ievt,893) = x3 * a3 * a2 * y2 * weightscale;
     WA(ievt,894) = x3 * a3 * a2 * b2 * weightscale;

     WA(ievt,895) = x3 * a3 * y2 * b2 * weightscale;

     WA(ievt,896) = x3 * y3 * b3 * x0 * weightscale;
     WA(ievt,897) = x3 * y3 * b3 * x2 * weightscale;
     WA(ievt,898) = x3 * y3 * b3 * a2 * weightscale;
     WA(ievt,899) = x3 * y3 * b3 * y2 * weightscale;
     WA(ievt,900) = x3 * y3 * b3 * b2 * weightscale;

     WA(ievt,901) = x3 * y3 * x0 * x2 * weightscale;
     WA(ievt,902) = x3 * y3 * x0 * a2 * weightscale;
     WA(ievt,903) = x3 * y3 * x0 * y2 * weightscale;
     WA(ievt,904) = x3 * y3 * x0 * b2 * weightscale;

     WA(ievt,905) = x3 * y3 * x2 * a2 * weightscale;
     WA(ievt,906) = x3 * y3 * x2 * y2 * weightscale;
     WA(ievt,907) = x3 * y3 * x2 * b2 * weightscale;

     WA(ievt,908) = x3 * y3 * a2 * y2 * weightscale;
     WA(ievt,909) = x3 * y3 * a2 * b2 * weightscale;

     WA(ievt,910) = x3 * y3 * y2 * b2 * weightscale;

     WA(ievt,911) = x3 * b3 * x0 * x2 * weightscale;
     WA(ievt,912) = x3 * b3 * x0 * a2 * weightscale;
     WA(ievt,913) = x3 * b3 * x0 * y2 * weightscale;
     WA(ievt,914) = x3 * b3 * x0 * b2 * weightscale;

     WA(ievt,915) = x3 * b3 * x2 * a2 * weightscale;
     WA(ievt,916) = x3 * b3 * x2 * y2 * weightscale;
     WA(ievt,917) = x3 * b3 * x2 * b2 * weightscale;

     WA(ievt,918) = x3 * b3 * a2 * y2 * weightscale;
     WA(ievt,919) = x3 * b3 * a2 * b2 * weightscale;

     WA(ievt,920) = x3 * b3 * y2 * b2 * weightscale;

     WA(ievt,921) = x3 * x0 * x2 * a2 * weightscale;
     WA(ievt,922) = x3 * x0 * x2 * y2 * weightscale;
     WA(ievt,923) = x3 * x0 * x2 * b2 * weightscale;

     WA(ievt,924) = x3 * x0 * a2 * y2 * weightscale;
     WA(ievt,925) = x3 * x0 * a2 * b2 * weightscale;

     WA(ievt,926) = x3 * x0 * y2 * b2 * weightscale;

     WA(ievt,927) = x3 * x2 * a2 * y2 * weightscale;
     WA(ievt,928) = x3 * x2 * a2 * b2 * weightscale;

     WA(ievt,929) = x3 * x2 * y2 * b2 * weightscale;

     WA(ievt,930) = x3 * a2 * y2 * b2 * weightscale;

     WA(ievt,931) = a3 * y3 * b3 * x0 * weightscale;
     WA(ievt,932) = a3 * y3 * b3 * x2 * weightscale;
     WA(ievt,933) = a3 * y3 * b3 * a2 * weightscale;
     WA(ievt,934) = a3 * y3 * b3 * y2 * weightscale;
     WA(ievt,935) = a3 * y3 * b3 * b2 * weightscale;

     WA(ievt,936) = a3 * y3 * x0 * x2 * weightscale;
     WA(ievt,937) = a3 * y3 * x0 * a2 * weightscale;
     WA(ievt,938) = a3 * y3 * x0 * y2 * weightscale;
     WA(ievt,939) = a3 * y3 * x0 * b2 * weightscale;

     WA(ievt,940) = a3 * y3 * x2 * a2 * weightscale;
     WA(ievt,941) = a3 * y3 * x2 * y2 * weightscale;
     WA(ievt,942) = a3 * y3 * x2 * b2 * weightscale;
     WA(ievt,943) = a3 * y3 * a2 * y2 * weightscale;
     WA(ievt,944) = a3 * y3 * a2 * b2 * weightscale;

     WA(ievt,945) = a3 * y3 * y2 * b2 * weightscale;

     WA(ievt,946) = a3 * b3 * x0 * x2 * weightscale;
     WA(ievt,947) = a3 * b3 * x0 * a2 * weightscale;
     WA(ievt,948) = a3 * b3 * x0 * y2 * weightscale;
     WA(ievt,949) = a3 * b3 * x0 * b2 * weightscale;

     WA(ievt,950) = a3 * b3 * x2 * a2 * weightscale;
     WA(ievt,951) = a3 * b3 * x2 * y2 * weightscale;
     WA(ievt,952) = a3 * b3 * x2 * b2 * weightscale;

     WA(ievt,953) = a3 * b3 * a2 * y2 * weightscale;
     WA(ievt,954) = a3 * b3 * a2 * b2 * weightscale;

     WA(ievt,955) = a3 * b3 * y2 * b2 * weightscale;

     WA(ievt,956) = a3 * x0 * x2 * a2 * weightscale;
     WA(ievt,957) = a3 * x0 * x2 * y2 * weightscale;
     WA(ievt,958) = a3 * x0 * x2 * b2 * weightscale;

     WA(ievt,959) = a3 * x0 * a2 * y2 * weightscale;
     WA(ievt,960) = a3 * x0 * a2 * b2 * weightscale;

     WA(ievt,961) = a3 * x0 * y2 * b2 * weightscale;

     WA(ievt,962) = a3 * x2 * a2 * y2 * weightscale;
     WA(ievt,963) = a3 * x2 * a2 * b2 * weightscale;

     WA(ievt,964) = a3 * x2 * y2 * b2 * weightscale;

     WA(ievt,965) = a3 * a2 * y2 * b2 * weightscale;

     WA(ievt,966) = y3 * b3 * x0 * x2 * weightscale;
     WA(ievt,967) = y3 * b3 * x0 * a2 * weightscale;
     WA(ievt,968) = y3 * b3 * x0 * y2 * weightscale;
     WA(ievt,969) = y3 * b3 * x0 * b2 * weightscale;

     WA(ievt,970) = y3 * b3 * x2 * a2 * weightscale;
     WA(ievt,971) = y3 * b3 * x2 * y2 * weightscale;
     WA(ievt,972) = y3 * b3 * x2 * b2 * weightscale;

     WA(ievt,973) = y3 * b3 * a2 * y2 * weightscale;
     WA(ievt,974) = y3 * b3 * a2 * b2 * weightscale;

     WA(ievt,975) = y3 * b3 * y2 * b2 * weightscale;

     WA(ievt,976) = y3 * x0 * x2 * a2 * weightscale;
     WA(ievt,977) = y3 * x0 * x2 * y2 * weightscale;
     WA(ievt,978) = y3 * x0 * x2 * b2 * weightscale;

     WA(ievt,979) = y3 * x0 * a2 * y2 * weightscale;
     WA(ievt,980) = y3 * x0 * a2 * b2 * weightscale;

     WA(ievt,981) = y3 * x0 * y2 * b2 * weightscale;

     WA(ievt,982) = y3 * x2 * a2 * y2 * weightscale;
     WA(ievt,983) = y3 * x2 * a2 * b2 * weightscale;

     WA(ievt,984) = y3 * x2 * y2 * b2 * weightscale;

     WA(ievt,985) = y3 * a2 * y2 * b2 * weightscale;

     WA(ievt,986) = b3 * x0 * x2 * a2 * weightscale;
     WA(ievt,987) = b3 * x0 * x2 * y2 * weightscale;
     WA(ievt,988) = b3 * x0 * x2 * b2 * weightscale;

     WA(ievt,989) = b3 * x0 * a2 * y2 * weightscale;
     WA(ievt,990) = b3 * x0 * a2 * b2 * weightscale;

     WA(ievt,991) = b3 * x0 * y2 * b2 * weightscale;

     WA(ievt,992) = b3 * x2 * a2 * y2 * weightscale;
     WA(ievt,993) = b3 * x2 * a2 * b2 * weightscale;

     WA(ievt,994) = b3 * x2 * y2 * b2 * weightscale;

     WA(ievt,995) = b3 * a2 * y2 * b2 * weightscale;

     WA(ievt,996) = x0 * x2 * a2 * y2 * weightscale;
     WA(ievt,997) = x0 * x2 * a2 * b2 * weightscale;

     WA(ievt,998) = x0 * x2 * y2 * b2 * weightscale;

     WA(ievt,999) = x0 * a2 * y2 * b2 * weightscale;

     WA(ievt,1000) = x2 * a2 * y2 * b2 * weightscale;
...
////   TFile *ofv = new TFile("output/mass/mass_calib_svd_4th_stm2_reflise_8step.hist_selected_unitlise_SSR_mahala.root","recreate");
////   TFile *ofv = new TFile("output/mass/mass_calib_svd_4th_stm2_reflise_8step.hist_selected_unitlise_SSR_mahala_cascmashi.root","recreate");
//   TFile *ofv = new TFile("output/mass/mass_calib_svd_4th_stm2_reflise_8step.hist_selected_unitlise_SSR_mahala_cascmashi_narrow.root","recreate");
////   TFile *ofv = new TFile("output/mass/mass_calib_svd_4th_stm2_reflise_8step.hist_selected_unitlise2_SSR_mahala.root","recreate");
////   TFile *ofv = new TFile("output/mass/mass_calib_svd_4th_stm2_reflise_8step.hist_selected_unitlise2_SSR_mahala_cascmashi.root","recreate");
//
...
//// Save the results of SVD
  ...
  Theta.Print();
   ...
   char foutname[50];
   std::ofstream foutmtr("output/mass/svd4th_out_stm2_reflise_8step_unitlise_SSR_mahala_incnew_w2_rsqrd.dat");
   ...
   std::ofstream foutmtrth("output/mass/theta_stm2_reflise_8step_unitlise_SSR_mahala_incnew_w2_rsqrd.dat");
 if (foutmtrth.fail()){
...
//// AoQ cor
  ...
  for (Int_t iData = 0; iData < nData; iData++){
    ...
    double sum = 0.;
	 ...
	 art::TMahalaData *mahala_cor = (art::TMahalaData*) array_mahala->UncheckedAt(0);
    ...
    Int_t hit_mahala_cor = array_mahala->GetEntriesFast();

     if ( hit_dia_tof32stmcor > 0 && hit_tracku_cor > 0 && hit_trackd_cor > 0 && hit_s0_cor >0 && hit_mahala_cor > 0){
     ...
     Double_t md_cor = mahala_cor->GetMd();

     Bool_t tracku_cor_ok = tracku_cor->IsGood();
     Bool_t trackd_cor_ok = trackd_cor->IsGood();
     Bool_t mahala_cor_ok = md_cor < 1;


    if( tracku_cor_ok && trackd_cor_ok  &&  mahala_cor_ok){
...
//// reflise2
//     Double_t t_cor = ((ti_cor - 2042.) * 0.025 + 0.35) * 3 + 1;  
//     Double_t x3_cor = x3i_cor * 0.1 + 1; 
//     Double_t a3_cor = a3i_cor * 0.1 + 1.5;
//     Double_t y3_cor = (y3i_cor + 2) * 0.5 + 1;
//     Double_t b3_cor = b3i_cor * 0.05 + 0.5 + 1;
//     Double_t x0_cor = x0i_cor * 0.015 + 1; 
//     Double_t x2_cor = x2i_cor * 0.1 + 1.5; 
//     Double_t a2_cor = a2i_cor * 0.025 + 1;
//     Double_t y2_cor = (y2i_cor * 0.05 - 1) * 0.5 + 1;
//     Double_t b2_cor = (b2i_cor * 0.05 - 0.2) * 0.5 + 1;
...
// 	   b_re_elem += A(ic,ir) * Theta(ir);
 ...
 double rss, tss, r_sqrd, rss_norm, sigma_coeff;

for(int i = 0; i < Nevt; i++){

 rss += (b_input(i,0) -  b_re(i)) * (b_input(i,0) -  b_re(i)); 
 tss += (b_input(i,0) -  b_ave) * (b_input(i,0) -  b_ave); 

}

 r_sqrd = 1 - ( rss / tss );
 rss_norm =  rss / (Nevt - Nprm);

 sigma_coeff = TMath::Sqrt(rss_norm);


 ofstream foutacc("/home/sh13/art_analysis/user/hanai/prm/mass/accuracy_stm2_reflise_8step_unit2.5_SSR_w_rsqrd.dat");
 if (foutacc.fail()){
	 cout << "Error; Could not open output file.." << endl << endl;
	 return;
  }

	foutacc << "RSS :" << rss << endl;
	foutacc << "R-sqrued :" << r_sqrd << endl;
	foutacc << "Sigma_coeff :" << sigma_coeff << endl;
...
// Output new mass region

 TH1D *h_aq_cal = new TH1D("aq_cal", "aq_cal",10000,1.7,1.96);
 TH2D *h_zaq_cal = new TH2D("zaq_cal", "zaq_cal",10000,1.7,1.96,1000,19,30);

 TH1D *hnew_ti_a = new TH1D("hnwe_ti_a","hnew_ti_a",5000,1.77,1.79); //39Ti

 TH1D *hnew_cr_a = new TH1D("hnwe_cr_a","hnew_cr_a",5000,1.74,1.76); //42Cr
 TH1D *hnew_cr_b = new TH1D("hnwe_cr_b","hnew_cr_d",5000,1.78,1.80); //43Cr

 TH1D *hnew_fe_a = new TH1D("hnwe_fe_a","hnew_fe_a",5000,1.72,1.74); //45Fe
 TH1D *hnew_fe_b = new TH1D("hnwe_fe_b","hnew_fe_d",5000,1.76,1.78); //46Fe
 TH1D *hnew_fe_c = new TH1D("hnwe_fe_c","hnew_fe_c",5000,1.79,1.81); //47Fe

 TH1D *hnew_ni_a = new TH1D("hnwe_ni_a","hnew_ni_a",5000,1.70,1.72); //48Ni
 TH1D *hnew_ni_b = new TH1D("hnwe_ni_b","hnew_ni_d",5000,1.74,1.76); //49Ni
 TH1D *hnew_ni_c = new TH1D("hnwe_ni_c","hnew_ni_c",5000,1.78,1.80); //50Ni
 TH1D *hnew_ni_d = new TH1D("hnwe_ni_d","hnew_ni_d",5000,1.81,1.83); //50Ni
...
// if (ssrd < 0.1){
 if (tracku_tmp_ok && trackd_tmp_ok){
    ...
    for (int s = 0; s < 35; s++){
      if(Z_low_draw[s] < z_tmpt && z_tmpt < Z_up_draw[s] && Aq_low_draw[s] < Aq_cor[iData] && Aq_cor[iData] <  Aq_up_draw[s]){
      
            h_zaq_cal->Fill(Aq_cor[iData],z_tmpt);
            h_aq_cal->Fill(Aq_cor[iData]); 
 ...
 if(21.5 < z_tmpt && z_tmpt < 22.5){

      hnew_ti_a->Fill(Aq_cor[iData]);

  }else if(23.5 < z_tmpt && z_tmpt < 24.5){

      hnew_cr_a->Fill(Aq_cor[iData]);
      hnew_cr_b->Fill(Aq_cor[iData]);

  }else if(25.5 < z_tmpt && z_tmpt < 26.5){

      hnew_fe_a->Fill(Aq_cor[iData]);
      hnew_fe_b->Fill(Aq_cor[iData]);
      hnew_fe_c->Fill(Aq_cor[iData]);

  }else if(27.5 < z_tmpt && z_tmpt < 28.5){

      hnew_ni_a->Fill(Aq_cor[iData]);
      hnew_ni_b->Fill(Aq_cor[iData]);
      hnew_ni_c->Fill(Aq_cor[iData]);
      hnew_ni_d->Fill(Aq_cor[iData]);
   ...
   TFile *ofile = new TFile("output/mass/mass_calib_svd_4th_eachstrip_stm2_reflise_8step_unitlise_SSR_mahala_incnew_w2_rsqrd.hist.root","recreate");
