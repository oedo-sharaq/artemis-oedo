...
void mass_calib7step_svd_stm_ref6_eventstore_toff(){

 TFile* file_load = TFile::Open("sh13_analysis/hanai/phys/bld_file/BLD_stm_tofoffset/physics.bld_stm_qcal_toff_6step.170272.all.root"); // Load file. Basically BLD file.
  if(!file_load){
...
// tree->SetBranchAddress("dia_tof32stm",&array_dia_tof32stm);
 tree->SetBranchAddress("dia_tof32stm_toff",&array_dia_tof32stm);
...
// tree->SetBranchStatus("dia_tof32stm",1);
 tree->SetBranchStatus("dia_tof32stm_toff",1);
...
// 4th order
 ...
 ofstream fout("/home/sh13/art_analysis/user/hanai/prm/mass/set_matrix_svd4th_stm2_ref6_7step_unit2.5_toff.dat");
// ofstream fout("/home/sh13/art_analysis/user/hanai/prm/mass/set_matrix_svd4th_stm2_ref6_7step_unit2.5_SSR_v2.dat");
 if (fout.fail()){
...
for(int n = 0; n < Nevt; n++){
 ...
 ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd4th_stm2_ref6_7step_unit2.5_toff.dat");
// ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd4th_stm2_ref6_7step_unit2.5_SSR_v2.dat");
 if (foutselect.fail()){
	 cout << "Error; Could not open output file.." << endl << endl;
	 return;
  }
...
for(int n = 0; n < Nevt; n++){
   ...
   TFile *ofv = new TFile("output/mass/mass_calib_svd_4th_stm2_ref6_7step.hist_selected_unit2.5_toff.root","recreate");
//   TFile *ofv = new TFile("output/mass/mass_calib_svd_4th_stm2_ref6_7step.hist_selected_unit2.5_SSR_v2.root","recreate");

  TH1D *tof_selected = new TH1D("tof_selected","tof_selected",1000,-1.5,1.5);
  TH2D *tofaq_selected = new TH2D("tofaq_selected","tofaq_selected",1000,1.8,2.15,1000,-1.5,1.5);
  TH1D *x3_selected = new TH1D("x3_selected","x3_selected",300,-1.5,1.5);
  TH2D *x3aq_selected = new TH2D("x3aq_selected","x3aq_selected",1000,1.8,2.2,300,-1.5,1.5);
  TH1D *a3_selected = new TH1D("a3_selected","a3_selected",300,-1.5,1.5);
  TH2D *a3aq_selected = new TH2D("a3aq_selected","a3aq_selected",1000,1.8,2.15,300,-1.5,1.5);
  TH1D *y3_selected = new TH1D("y3_selected","y3_selected",300,-1.5,1.5);
  TH2D *y3aq_selected = new TH2D("y3aq_selected","y3aq_selected",1000,1.8,2.15,300,-1.5,1.5);
  TH1D *b3_selected = new TH1D("b3_selected","b3_selected",300,-1.5,1.5);
  TH2D *b3aq_selected = new TH2D("b3aq_selected","b3aq_selected",100,1.8,2.15,300,-1.5,1.5);
  TH1D *x0_selected = new TH1D("x0_selected","x0_selected",300,-1.5,1.5);
  TH2D *x0aq_selected = new TH2D("x0aq_selected","x0aq_selected",1000,1.8,2.15,300,-1.5,1.5);
  TH1D *x2_selected = new TH1D("x2_selected","x2_selected",300,-1.5,1.5);
  TH2D *x2aq_selected = new TH2D("x2aq_selected","x2aq_selected",1000,1.8,2.15,300,-1.5,1.5);
  TH1D *a2_selected = new TH1D("a2_selected","a2_selected",300,-1.5,1.5);
  TH2D *a2aq_selected = new TH2D("a2aq_selected","a2aq_selected",1000,1.8,2.15,300,-1.5,1.5);
  TH1D *y2_selected = new TH1D("y2_selected","y2_selected",300,-1.5,1.5);
  TH2D *y2aq_selected = new TH2D("y2aq_selected","y2aq_selected",1000,1.8,2.15,300,-1.5,1.5);
  TH2D *y2ssr_selected = new TH2D("y2ssr_selected","y2ssr_select5ed",300,-1.5,1.5,300,0,1);
  TH1D *b2_selected = new TH1D("b2_selected","b2_selected",300,-1.5,1.5);
  TH2D *b2aq_selected = new TH2D("b2aq_selected","b2aq_selected",1000,1.8,2.15,300,-1.5,1.5);
  TH2D *b2ssr_selected = new TH2D("b2ssr_selected","b2ssr_selected",300,-1.5,1.5,300,0,1);
...
//// Save the results of SVD
  ...
  Theta.Print();
   ...
   char foutname[50];
   std::ofstream foutmtr("output/mass/svd4th_out_stm2_ref6_7step_unit2.5_toff.dat");
//   std::ofstream foutmtr("output/mass/svd4th_out_stm2_ref6_7step_unit2.5_SSR_v2.dat");
   ...
   std::ofstream foutmtrth("output/mass/theta_stm2_ref6_7step_unit2.5_toff.dat");
//   std::ofstream foutmtrth("output/mass/theta_stm2_ref6_7step_unit2.5_SSR_v2.dat");
 if (foutmtrth.fail()){
...
//// AoQ cor
  ...
  for (Int_t iData = 0; iData < nData; iData++){
    ...
    Double_t ssrd_cor = trackd_cor->GetSSR();

 //   if ( hit_dia_tof32stmcor > 0 && hit_tracku_cor > 0 && hit_trackd_cor > 0 && hit_s0_cor >0 && ssrd_cor < 0.1 ){
...
//// Draw original histogram
 ...
 TH2D *h_zaq_org = new TH2D("zaq_org", "zaq_org",500,1.7,2.3,500,18,30);
  ...
  for (Int_t iData = 0; iData < nData; iData++){
   ...
   TFile *ofile = new TFile("output/mass/mass_calib_svd_4th_eachstrip_stm2_ref6_7step_unit2.5_SSR_toff.hist.root","recreate");
