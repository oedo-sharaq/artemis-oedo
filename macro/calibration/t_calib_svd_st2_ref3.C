...
void t_calib_svd_st2(){

// TFile* file = TFile::Open("sh13_analysis/hanai/phys/merge/physics_merge_ref_eachstrip.all.170272.root");
 TFile* file = TFile::Open("sh13_analysis/hanai/phys/bld_file/physics.bld.170272.all.root");
// TFile* file = TFile::Open("sh13_analysis/hanai/phys/bld_file/BLD_ssdpla/BLD_tpla_ssdpla.170272.all.root");
 if(!file){
...
//  const Int_t Nrun = 250;  //the number of runs
 ...
 TClonesArray *array_mass_st2 = NULL;
...
// tree->SetBranchAddress("mass_zcal",&array_mass);
 tree->SetBranchAddress("mass_st2",&array_mass_st2);
...
// tree->SetBranchStatus("mass_zcal",1);
 ...
 tree->SetBranchStatus("mass_st2",1);
...
// ifstream fin("/home/sh13/art_analysis/user/hanai/prm/mass/input_try.dat");
 ifstream fin("/home/sh13/art_analysis/user/hanai/prm/mass/input_try_ref3.dat");
// ifstream fin("/home/sh13/art_analysis/user/hanai/prm/mass/input_try_ssdpla.dat");
 if (fin.fail()){
...
//// Fill date 
 ...
 cout << "Number of entries is  " << nData << endl;
...
for (int iref = 0; iref < Nref; iref++){
  ...
  for(int i = 0; i < nData; i++){
      ...
      Int_t hit_mass_st2 = array_mass_st2->GetEntriesFast();
      Int_t hit_tracku = array_tracku->GetEntriesFast();
      Int_t hit_s0 = array_s0->GetEntriesFast();

     if ( hit_mass > 0 && hit_mass_st2 > 0 && hit_tracku > 0 && hit_s0 > 0){

	 art::TMassData *mass_tmp = (art::TMassData*) array_mass->UncheckedAt(0);
	 art::TMassData *mass_st2_tmp = (art::TMassData*) array_mass_st2->UncheckedAt(0);
     Double_t z_tmp = mass_tmp->GetZ();
     Double_t aq_tmp = mass_st2_tmp->GetAq();
...
//// Fill data in matrix for each Ref	 
   for(int p = 0; p < Nrun; p++){
	 ...
	 art::TMassData *mass_st2 = (art::TMassData*) array_mass_st2->UncheckedAt(0);
     ...
     Double_t aq_select = mass_st2->GetAq();
 
     Double_t ti = mass_st2->GetTof();
...
// ofstream fout("/home/sh13/art_analysis/user/hanai/prm/mass/set_matrix_svd3rd_eachstripst2.dat");
 ofstream fout("/home/sh13/art_analysis/user/hanai/prm/mass/set_matrix_svd3rd_eachstripst2_BLD_ref3.dat");
// ofstream fout("/home/sh13/art_analysis/user/hanai/prm/mass/set_matrix_svd3rd_eachstripst2_ssdpla_.dat");
 if (fout.fail()){
...
// ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd3rd_eachstripst2.dat");
 ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd3rd_eachstripst2_BLD_ref3.dat");
// ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd3rd_eachstripst2_ssdpla.dat");
 if (foutselect.fail()){
	 cout << "Error; Could not open output file.." << endl << endl;
	 return;
  }
...
//// Save the results of SVD
  ...
  Theta.Print();
   ...
   char foutname[50];
//   std::ofstream foutmtr("output/mass/svd3rd_out_st2.dat");
   std::ofstream foutmtr("output/mass/svd3rd_out_st2_BLD_ref3.dat");
//   std::ofstream foutmtr("output/mass/svd3rd_out_st2_ssdpla.dat");
...
//   std::ofstream foutmtrth("output/mass/theta_st2.dat");
   std::ofstream foutmtrth("output/mass/theta_st2_BLD_ref3.dat");
//   std::ofstream foutmtrth("output/mass/theta_st2_ssdpla.dat");
 if (foutmtrth.fail()){
...
//// AoQ cor
  ...
  for (Int_t iData = 0; iData < nData; iData++){
    double sum = 0.;
     ...
     tree->GetEntry(iData);
	 art::TMassData *mass_cor = (art::TMassData*) array_mass_st2->UncheckedAt(0);
    ...
    Int_t hit_mass_cor = array_mass_st2->GetEntriesFast();
...
// TH1D *h_45V_org = new TH1D("h45V_org", "h45V_org",51000,1.945,1.965);
 ...
 TH2D *h_zaq_org = new TH2D("zaq_org", "zaq_org",500,1.7,2.3,500,18,30);
  ...
  for (Int_t iData = 0; iData < nData; iData++){
     tree->GetEntry(iData);
     art::TMassData *mass_tmpt = (art::TMassData*) array_mass->UncheckedAt(0);
     art::TMassData *mass_corst2_tmpt = (art::TMassData*) array_mass_st2->UncheckedAt(0);
     Double_t z_tmpt = mass_tmpt->GetZ();
...
//   TFile *ofile = new TFile("output/mass/mass_calib_svd_3rd_eachtrip_st2.hist.root","recreate");
   TFile *ofile = new TFile("output/mass/mass_calib_svd_3rd_eachtrip_st2_BLD.hist_ref3.root","recreate");
//   TFile *ofile = new TFile("output/mass/mass_calib_svd_3rd_eachtrip_st2_ssdpla.hist.root","recreate");
