...
void mass_calib2step_svd_st0(){

 TFile* file = TFile::Open("sh13_analysis/hanai/phys/merge/physics.chkmass_st0.170272.all.root");
 if(!file){
...
//  const Int_t Nrun = 2500;  //the number of runs
 ...
 TClonesArray *array_dia_tof32st0 = NULL;
 TClonesArray *array_mass_corst0 = NULL;
 TClonesArray *array_tracku = NULL;
 TClonesArray *array_s0 = NULL;
 TTree *tree = (TTree*) gDirectory->Get("tree");

 tree->SetBranchAddress("mass",&array_mass);
 tree->SetBranchAddress("dia_tof32st0",&array_dia_tof32st0);
 tree->SetBranchAddress("mass_corst0",&array_mass_corst0);
 tree->SetBranchAddress("f3",&array_tracku);
 tree->SetBranchAddress("sr0_xcw",&array_s0);
 tree->SetBranchStatus("*",0);
 tree->SetBranchStatus("mass",1);
 tree->SetBranchStatus("dia_tof32st0",1);
 tree->SetBranchStatus("mass_corst0",1);
 ...
 ifstream fin("/home/sh13/art_analysis/user/hanai/prm/mass/input_2step_st0.dat");
 if (fin.fail()){
...
for (int iref = 0; iref < Nref; iref++){
  ...
  for(int i = 0; i < nData; i++){
      ...
      Int_t hit_mass_corst0 = array_mass_corst0->GetEntriesFast();
      Int_t hit_tracku = array_tracku->GetEntriesFast();
      Int_t hit_s0 = array_s0->GetEntriesFast();

     if ( hit_mass > 0 && hit_mass_corst0 > 0 && hit_tracku > 0 && hit_s0 > 0){

	 art::TMassData *mass_tmp = (art::TMassData*) array_mass->UncheckedAt(0);
	 art::TMassData *mass_corst0_tmp = (art::TMassData*) array_mass_corst0->UncheckedAt(0);
     Double_t z_tmp = mass_tmp->GetZ();
     Double_t aq_tmp = mass_corst0_tmp->GetAq();
  ...
  if (n_tmp < Nrun){
    cout << "Caution!! Reference No." << iref << " does not have enough number of events. (only "<<  n_tmp << " events.) \n";
    HasEvents = false;  
	continue;
    }else{
...
//// Fill data in matrix for each Ref	 
   for(int p = 0; p < Nrun; p++){
	 ...
	 art::TDiaTimingData *dia_tof32st0 = (art::TDiaTimingData*) array_dia_tof32st0->UncheckedAt(0);
	 art::TMassData *mass_corst0 = (art::TMassData*) array_mass_corst0->UncheckedAt(0);
     ...
     Double_t aq_select = mass_corst0->GetAq();
 
     Double_t ti = dia_tof32st0->GetTiming();
...
// unit orientation
 ...
 ofstream fout("/home/sh13/art_analysis/user/hanai/prm/mass/set_matrix_svd3rd_eachstripst0_ref1_2step.dat");
 if (fout.fail()){
...
for(int n = 0; n < Nevt; n++){
 ...
 ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd3rd_eachstripst0_ref1_2step.dat");
 if (foutselect.fail()){
	 cout << "Error; Could not open output file.." << endl << endl;
	 return;
  }
...
//// Save the results of SVD
  ...
  Theta.Print();
   ...
   char foutname[50];
   std::ofstream foutmtr("output/mass/svd3rd_out_st0_ref1_2step.dat");
   ...
   std::ofstream foutmtrth("output/mass/theta_st0_ref1_2step.dat");
 if (foutmtrth.fail()){
...
//// AoQ cor
  ...
  for (Int_t iData = 0; iData < nData; iData++){
    double sum = 0.;
     ...
     tree->GetEntry(iData);
	 art::TDiaTimingData *dia_tof32st0cor = (art::TDiaTimingData*) array_dia_tof32st0->UncheckedAt(0);
    ...
    Int_t hit_dia_tof32st0cor = array_dia_tof32st0->GetEntriesFast();
    Int_t hit_tracku_cor = array_tracku->GetEntriesFast();
    Int_t hit_s0_cor = array_s0->GetEntriesFast();

     if ( hit_dia_tof32st0cor > 0 && hit_tracku_cor > 0 && hit_s0_cor >0){
 
     Double_t ti_cor = dia_tof32st0cor->GetTiming();
...
//// Draw original histogram
 ...
 TH2D *h_zaq_org = new TH2D("zaq_org", "zaq_org",500,1.7,2.3,500,18,30);
  ...
  for (Int_t iData = 0; iData < nData; iData++){
     tree->GetEntry(iData);
     art::TMassData *mass_tmpt = (art::TMassData*) array_mass->UncheckedAt(0);
     art::TMassData *mass_corst0_tmpt = (art::TMassData*) array_mass_corst0->UncheckedAt(0);
     Double_t z_tmpt = mass_tmpt->GetZ();
     Double_t aq_tmpt = mass_corst0_tmpt->GetAq();
   ...
   TFile *ofile = new TFile("output/mass/mass_calib_svd_3rd_eachstrip_st0_ref1_2step.hist.root","recreate");
