...
void mass_calib2step_svd_st1(){

 TFile* file = TFile::Open("sh13_analysis/hanai/phys/merge/physics.chkmass_st1.170272.all.root");
 if(!file){
...
//  const Int_t Nrun = 2500;  //the number of runs
 ...
 tree->SetBranchAddress("mass_corst1",&array_mass_corst1);
 ...
 tree->SetBranchStatus("mass_corst1",1);
...
for(int n = 0; n < Nevt; n++){
 ...
 ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd3rd_eachstripst1_ref1_2step.dat");
 if (foutselect.fail()){
	 cout << "Error; Could not open output file.." << endl << endl;
	 return;
  }
...
//// Save the results of SVD
  ...
  Theta.Print();
   ...
   char foutname[50];
   std::ofstream foutmtr("output/mass/svd3rd_out_st1_ref1_2step.dat");
   ...
   std::ofstream foutmtrth("output/mass/theta_st1_ref1_2step.dat");
 if (foutmtrth.fail()){
...
//// Draw original histogram
 ...
 TH2D *h_zaq_org = new TH2D("zaq_org", "zaq_org",500,1.7,2.3,500,18,30);
  ...
  for (Int_t iData = 0; iData < nData; iData++){
   ...
   TFile *ofile = new TFile("output/mass/mass_calib_svd_3rd_eachstrip_st1_ref1_2step.hist.root","recreate");
