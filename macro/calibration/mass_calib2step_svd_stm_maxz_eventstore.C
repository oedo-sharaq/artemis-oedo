...
void mass_calib2step_svd_stm_maxz_eventstore(){

 TFile* file_load = TFile::Open("sh13_analysis/hanai/phys/mass_cor_maxz/physics.chkmass_stm2_1step_ref4_maxz.eventstore.170272.all.root");
  if(!file_load){
...
// unit orientation
 ...
 ofstream fout("/home/sh13/art_analysis/user/hanai/prm/mass/set_matrix_svd3rd_stm2_ref4_2step_BLD_maxz.dat");
 if (fout.fail()){
...
for(int n = 0; n < Nevt; n++){
 ...
 ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd3rd_stm2_ref4_2step_BLD_maxz.dat");
 if (foutselect.fail()){
	 cout << "Error; Could not open output file.." << endl << endl;
	 return;
  }
...
//// Save the results of SVD
  ...
  Theta.Print();
   ...
   char foutname[50];
   std::ofstream foutmtr("output/mass/svd3rd_out_stm2_ref4_2step_BLD_maxz.dat");
   ...
   std::ofstream foutmtrth("output/mass/theta_stm2_ref4_2step_BLD_maxz.dat");
 if (foutmtrth.fail()){
...
//      h_47V_org->Fill(aq_tmpt);
  ...
  TFile *ofile = new TFile("output/mass/mass_calib_svd_3rd_eachstrip_stm2_ref4_2step_BLD_maxz.hist.root","recreate");
