...
void chkmass_calib7step_svd_stm_reflise_eventstore(){
...
Double_t Ref_m_tmp, Z_low_tmp, Z_up_tmp, Aq_low_tmp, Aq_up_tmp;
 ...
 Double_t Theta_tmp;
 vector<Double_t> Theta_input;

// ifstream ftheta("/home/sh13/art_analysis/user/hanai/solution/output_files/theta_coefficients_linalgplspinv.csv");
// ifstream ftheta("/home/sh13/art_analysis/user/hanai/solution/output_files/theta_coefficients_ridge_optimal.csv");
 ifstream ftheta("/home/sh13/art_analysis/user/hanai/solution/output_files/theta_coefficients_svd.csv");
 if (ftheta.fail()){
	 cout << "Error; inputfile" << " " << "is not found." << endl << endl;
	 return;
  }
  ...
  while(!ftheta.eof()){
	    ftheta.precision(10);
	    ftheta >> Theta_tmp;
		Theta_input.push_back(Theta_tmp);
		}
 ftheta.close();
...
cout << "Theta values are following" << endl; 

for(int i = 0; i < Nprm; i++){
  
  cout << " Theta( " << i << ")  : " <<  Theta_input[i] << endl;
...
//// AoQ cor
cout << "Start mass calcuration" << endl; 
...
//   TFile *ofile = new TFile("output/mass/chkmass_linalgplspinv_stm2_reflise_8step_unitlise_SSR_epsilon_angle.hist.root","recreate");
//   TFile *ofile = new TFile("output/mass/chkmass_ridge_optimal_stm2_reflise_8step_unitlise_SSR_epsilon_angle.hist.root","recreate");
   TFile *ofile = new TFile("output/mass/chkmass_svd_stm2_reflise_8step_unitlise_SSR_epsilon_angle.hist.root","recreate");
