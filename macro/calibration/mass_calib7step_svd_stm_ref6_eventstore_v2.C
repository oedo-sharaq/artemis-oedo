...
void mass_calib7step_svd_stm_ref6_eventstore_v2(){

 TFile* file_load = TFile::Open("sh13_analysis/hanai/phys/bld_file/physics.bld_stm_v2.170272.all.root"); // Load file. Basically BLD file.
  if(!file_load){
...
//// Draw original histogram
 ...
 TH2D *h_zaq_org = new TH2D("zaq_org", "zaq_org",500,1.7,2.3,500,18,30);
  ...
  for (Int_t iData = 0; iData < nData; iData++){
   ...
   TFile *ofile = new TFile("output/mass/mass_calib_svd_4th_eachstrip_stm2_ref6_7step_unit2.5_SSR_v2.hist.root","recreate");
