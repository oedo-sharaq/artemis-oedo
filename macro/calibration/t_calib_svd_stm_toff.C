...
//  const Int_t Nrun = 250;  //the number of runs
 ...
 TClonesArray *array_mass_stm = NULL;
...
// tree->SetBranchAddress("mass_zcal",&array_mass);
 tree->SetBranchAddress("mass_stm",&array_mass_stm);
...
// tree->SetBranchStatus("mass_zcal",1);
 ...
 tree->SetBranchStatus("mass_stm",1);
...
for (int iref = 0; iref < Nref; iref++){
  ...
  for(int i = 0; i < nData; i++){
      ...
      Int_t hit_mass_stm = array_mass_stm->GetEntriesFast();
      Int_t hit_tracku = array_tracku->GetEntriesFast();
      Int_t hit_s0 = array_s0->GetEntriesFast();

     if ( hit_mass > 0 && hit_mass_stm > 0 && hit_tracku > 0 && hit_s0 > 0){

	 art::TMassData *mass_tmp = (art::TMassData*) array_mass->UncheckedAt(0);
	 art::TMassData *mass_stm_tmp = (art::TMassData*) array_mass_stm->UncheckedAt(0);
     Double_t z_tmp = mass_tmp->GetZ();
     Double_t aq_tmp = mass_stm_tmp->GetAq();
...
//// Fill data in matrix for each Ref	 
   for(int p = 0; p < Nrun; p++){
	 ...
	 art::TMassData *mass_stm = (art::TMassData*) array_mass_stm->UncheckedAt(0);
     ...
     Double_t aq_select = mass_stm->GetAq();
 
     Double_t ti = mass_stm->GetTof();
...
// unit orientation
 ...
 ofstream fout("/home/sh13/art_analysis/user/hanai/prm/mass/set_matrix_svd3rd_stm2_BLD_ref4.dat");
 if (fout.fail()){
...
// ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd3rd_eachstripstm.dat");
 ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd3rd_stm2_BLD_ref3.dat");
// ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd3rd_eachstripstm_ssdpla.dat");
 if (foutselect.fail()){
	 cout << "Error; Could not open output file.." << endl << endl;
	 return;
  }
...
//// Save the results of SVD
  ...
  Theta.Print();
   ...
   char foutname[50];
//   std::ofstream foutmtr("output/mass/svd3rd_out_stm.dat");
   std::ofstream foutmtr("output/mass/svd3rd_out_stm2_BLD_ref4.dat");
//   std::ofstream foutmtr("output/mass/svd3rd_out_stm_ssdpla.dat");
...
//   std::ofstream foutmtrth("output/mass/theta_stm.dat");
   std::ofstream foutmtrth("output/mass/theta_stm2_BLD_ref4.dat");
//   std::ofstream foutmtrth("output/mass/theta_stm_ssdpla.dat");
 if (foutmtrth.fail()){
...
//// AoQ cor
  ...
  for (Int_t iData = 0; iData < nData; iData++){
    double sum = 0.;
     ...
     tree->GetEntry(iData);
	 art::TMassData *mass_cor = (art::TMassData*) array_mass_stm->UncheckedAt(0);
    ...
    Int_t hit_mass_cor = array_mass_stm->GetEntriesFast();
...
// TH1D *h_47V_org = new TH1D("h47V_org", "h47V_org",51000,2.01,2.21);
 ...
 TH2D *h_zaq_org = new TH2D("zaq_org", "zaq_org",500,1.7,2.3,500,18,30);
  ...
  for (Int_t iData = 0; iData < nData; iData++){
     tree->GetEntry(iData);
     art::TMassData *mass_tmpt = (art::TMassData*) array_mass->UncheckedAt(0);
     art::TMassData *mass_corstm_tmpt = (art::TMassData*) array_mass_stm->UncheckedAt(0);
     Double_t z_tmpt = mass_tmpt->GetZ();
...
//      h_47V_org->Fill(aq_tmpt);
   ...
   TFile *ofile = new TFile("output/mass/mass_calib_svd_3rd_eachtrip_stm2_BLD.hist_ref4.root","recreate");
