...
//  const Int_t Nrun = 250;  //the number of runs
 ...
 ifstream fin("/home/sh13/art_analysis/user/hanai/prm/mass/input_try_st0.dat");
 if (fin.fail()){
...
for (int iref = 0; iref < Nref; iref++){
  ...
  for(int i = 0; i < nData; i++){
      ...
      Int_t hit_mass_st0 = array_mass_st0->GetEntriesFast();
      Int_t hit_tracku = array_tracku->GetEntriesFast();
      Int_t hit_s0 = array_s0->GetEntriesFast();

     if ( hit_mass > 0 && hit_mass_st0 > 0 && hit_tracku > 0 && hit_s0 > 0 ){

	 art::TMassData *mass_tmp = (art::TMassData*) array_mass->UncheckedAt(0);
	 art::TMassData *mass_corst0_tmp = (art::TMassData*) array_mass_st0->UncheckedAt(0);
     Double_t z_tmp = mass_tmp->GetZ();
     Double_t aq_tmp = mass_cosrst0_tmp->GetAq();
...
//// Fill data in matrix for each Ref	 
   for(int p = 0; p < Nrun; p++){
	 ...
	 art::TMassData *mass_st0 = (art::TMassData*) array_mass_st0->UncheckedAt(0);
     ...
     Double_t aq_select = mass_st0->GetAq();
 
     Double_t ti = mass_st0->GetTof();
...
// unit orientation
 ...
 ofstream fout("/home/sh13/art_analysis/user/hanai/prm/mass/set_matrix_svd3rd_eachstripst0.dat");
 if (fout.fail()){
...
for(int n = 0; n < Nevt; n++){
 ...
 ofstream foutselect("/home/sh13/art_analysis/user/hanai/prm/mass/value_selected_svd3rd_eachstripst0.dat");
 if (foutselect.fail()){
	 cout << "Error; Could not open output file.." << endl << endl;
	 return;
  }
...
//// Save the results of SVD
  ...
  Theta.Print();
   ...
   char foutname[50];
   std::ofstream foutmtr("output/mass/svd3rd_out_st0.dat");
   ...
   std::ofstream foutmtrth("output/mass/theta_st0.dat");
 if (foutmtrth.fail()){
...
//// AoQ cor
  ...
  for (Int_t iData = 0; iData < nData; iData++){
    double sum = 0.;
     ...
     tree->GetEntry(iData);
	 art::TMassData *mass_cor = (art::TMassData*) array_mass_st0->UncheckedAt(0);
    ...
    Int_t hit_mass_cor = array_mass_st0->GetEntriesFast();
...
//// Draw original histogram
 ...
 TH2D *h_zaq_org = new TH2D("zaq_org", "zaq_org",500,1.7,2.3,500,18,30);
  ...
  for (Int_t iData = 0; iData < nData; iData++){
   ...
   TFile *ofile = new TFile("output/mass/mass_calib_svd_3rd_eachstrip_st0.hist.root","recreate");
