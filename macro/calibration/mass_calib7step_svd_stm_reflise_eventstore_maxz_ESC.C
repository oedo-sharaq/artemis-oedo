...
void mass_calib7step_svd_stm_reflise_eventstore_maxz_ESC(){
...
//	foutselect.close();
   ...
   TFile *ofv = new TFile("output/mass/mass_calib_svd_4th_st2_reflise_7step.hist_selected_unitlise_SSR_maxz.root","recreate");
...
//// Save the results of SVD
  ...
  Theta.Print();
   ...
   char foutname[50];
   std::ofstream foutmtr("output/mass/svd4th_out_st2_reflise_7step_unitlise_SSR.dat");
   ...
   std::ofstream foutmtrth("output/mass/theta_st2_reflise_8step_unitlise_SSR.dat");
 if (foutmtrth.fail()){
...
//    if( tracku_cor_ok && trackd_cor_ok  && angle_ok){
    if( tracku_cor_ok && trackd_cor_ok  && dia_id_2){
...
//// Draw original histogram
 ...
 TH2D *h_zaq_org = new TH2D("zaq_org", "zaq_org",1000,1.7,2.3,1000,18,30);
  ...
  for (Int_t iData = 0; iData < nData; iData++){
   ...
   TFile *ofile = new TFile("output/mass/mass_calib_svd_4th_eachstrip_st2_reflise_8step_unitlise_SSR_maxz.hist.root","recreate");
