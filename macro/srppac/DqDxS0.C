...
//upstream
  ...
  TH1D *hsrux = (TH1D*)gROOT->FindObject("sr0_x_q0q1");

  double totsrux=hsrux->Integral();
  hsrux->Scale(1./totsrux);

  char filenamesrux[50];
  sprintf(filenamesrux,"prm/srppac/sr0/dq2dx/%s/xc0.dat",runnumber);
...
//   }
  ...
  TH1D *hsruy = (TH1D*)gROOT->FindObject("sr0_y_q0q1");

  double totsruy=hsruy->Integral();
  hsruy->Scale(1./totsruy);

  char filenamesruy[50];
  sprintf(filenamesruy,"prm/srppac/sr0/dq2dx/%s/yc0.dat",runnumber);
