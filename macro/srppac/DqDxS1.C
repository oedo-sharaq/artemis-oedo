...
//downstream
  ...
  char mkdd[30];
  sprintf(mkdd,".! mkdir -p prm/srppac/sr12/dq2dx/%s",runnumber);

  gROOT->ProcessLine(mkdd);


  TH1D *hsrdx = (TH1D*)gROOT->FindObject("sr12_x_q0q1");

  double totsrdx=hsrdx->Integral();
  hsrdx->Scale(1./totsrdx);

  char filenamesrdx[50];
  sprintf(filenamesrdx,"prm/srppac/sr12/dq2dx/%s/xc0.dat",runnumber);
...
//   }
  ...
  TH1D *hsrdy = (TH1D*)gROOT->FindObject("sr12_y_q0q1");

  double totsrdy=hsrdy->Integral();
  hsrdy->Scale(1./totsrdy);

  char filenamesrdy[50];
  sprintf(filenamesrdy,"prm/srppac/sr12/dq2dx/%s/yc0.dat",runnumber);
