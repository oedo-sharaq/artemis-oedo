...
//upstream L
  ...
  TH1D *hsruxl = (TH1D*)gROOT->FindObject("sr0_x_q0ql");

  double totsruxl=hsruxl->Integral();
  hsruxl->Scale(1./totsruxl);

  char filenamesruxl[50];
  sprintf(filenamesruxl,"prm/srppac/sr0/dq2dx/%s/xcl.dat",runnumber);
...
//upstream R
  ...
  TH1D *hsruxr = (TH1D*)gROOT->FindObject("sr0_x_q0qr");

  double totsruxr=hsruxr->Integral();
  hsruxr->Scale(1./totsruxr);

  char filenamesruxr[50];
  sprintf(filenamesruxr,"prm/srppac/sr0/dq2dx/%s/xcr.dat",runnumber);
