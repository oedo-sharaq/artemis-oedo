...
void massfit_v_ref5(){
 ...
 ofstream fout1("/home/sh13/art_analysis/user/hanai/prm/mass/strip1/ref5_fitting_st1_3step_SSR.eventstore.43v.dat");
 if(fout1.fail()){
 ...
 ofstream fout2("/home/sh13/art_analysis/user/hanai/prm/mass/strip1/ref5_fitting_st1_3step_SSR.eventstore.44v.dat");
 if(fout2.fail()){
	 cout << "Error; Could not open output file.." << endl << endl;
	 gROOT->ProcessLine(".q");
	 return;
 ...
 ofstream fout3("/home/sh13/art_analysis/user/hanai/prm/mass/strip1/ref5_fitting_st1_3step_SSR.eventstore.46v.dat");
 if(fout3.fail()){
	 cout << "Error; Could not open output file.." << endl << endl;
	 gROOT->ProcessLine(".q");
	 return;
...
//}else{
 ...
 fout3 << "[fit scale]    [fit mean]    [fit sigma]   [Entry]    [A/Q mean +- 3sigma]    [Mass mean +- 3sigma (keV)] [Mass mean fit error (keV)]  [3sigma (keV)]   [aq diff]  [mass diff (keV)]  [TOF diff (ps)] [TOF fit error (ps)]" << endl;  
  fout3  << "  " << prm3[0] << "  " << prm3[1] << "  " << prm3[2] << "  " << nh46v << "  " << mean3 << "  " << mass3_keV << "  " << "  " << mass3_keV_error << "  " << reso3 << "  " << aq_diff3 << "  " << massdiff3 << "  " << tdiff3 << "  " << t_46v_error << endl;
