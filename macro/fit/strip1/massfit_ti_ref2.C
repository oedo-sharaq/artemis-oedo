...
// ofstream fout1("/home/sh13/art_analysis/user/hanai/prm/mass/strip1/ref_fitting_st1_4.5step_ssdpla_nooff.eventstore.40ti.dat");
 ofstream fout1("/home/sh13/art_analysis/user/hanai/prm/mass/strip1/ref2_fitting_st1_3step.eventstore.40ti.dat");
 if(fout1.fail()){
...
// ofstream fout2("/home/sh13/art_analysis/user/hanai/prm/mass/strip1/ref_fitting_st1_4.5step_ssdpla_nooff.eventstore.41ti.dat");
 ofstream fout2("/home/sh13/art_analysis/user/hanai/prm/mass/strip1/ref2_fitting_st1_3step.eventstore.41ti.dat");
 if(fout2.fail()){
	 cout << "Error; Could not open output file.." << endl << endl;
	 gROOT->ProcessLine(".q");
	 return;
...
// ofstream fout3("/home/sh13/art_analysis/user/hanai/prm/mass/strip1/ref_fitting_st1_4.5step_ssdpla_nooff.eventstore.47ti.dat");
 ofstream fout3("/home/sh13/art_analysis/user/hanai/prm/mass/strip1/ref2_fitting_st1_3step.eventstore.47ti.dat");
 if(fout3.fail()){
	 cout << "Error; Could not open output file.." << endl << endl;
	 gROOT->ProcessLine(".q");
	 return;
...
TH1D *h47ti = (TH1D*)gDirectory->Get("h47ti_aq");
 Int_t nh47ti = h47ti->GetEntries();
 h47ti->Fit("gaus","L","",2.132,2.135);
// if( nh47ti < 200 ){
//  fout3 << i << " " << 0 << " " << 0 << " " << 0 << " " << nh47ti <<  endl;
// delete gROOT->Get("h47ti");
//}else{
 TF1 *f3 = h47ti->GetFunction("gaus");
 ...
 h47ti->GetXaxis()->SetRangeUser(xmin3,xmax3);
 Double_t mean3 = h47ti->GetMean();
 ...
 Double_t aq_diff3 = mean3 - ref_47ti;
 Double_t massdiff3 =  aq_diff3 * amu * z;
 Double_t t_47ti = tof_func(L,ele,z,Brho,c,mass3); // [ps]
 Double_t t_47ti_error = tof_func(L,ele,z,Brho,c,mass3) * mass3_error; // [ps]
 Double_t tdiff3 = t_47ti - t_ref_47ti;
 fout3 << "[fit scale]    [fit mean]    [fit sigma]   [Entry]    [A/Q mean +- 3sigma]    [Mass mean +- 3sigma (keV)] [Mass mean fit error (keV)]  [3sigma (keV)]   [aq diff]  [mass diff (keV)]  [TOF diff (ps)] [TOF fit error (ps)]" << endl;  
  fout3  << "  " << prm3[0] << "  " << prm3[1] << "  " << prm3[2] << "  " << nh47ti << "  " << mean3 << "  " << mass3_keV << "  " << "  " << mass3_keV_error << "  " << reso3 << "  " << aq_diff3 << "  " << massdiff3 << "  " << tdiff3 << "  " << t_47ti_error << endl;
