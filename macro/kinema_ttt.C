...
void kinema_ttt(Int_t a1, Int_t z1,
            Int_t a2, Int_t z2,
            Int_t a3, Int_t z3,
            ...
            Double_t exmin, Double_t exmax, Double_t estep)
{
    ...
    // TH2F *h2 = new TH2F("h","",80,100.,180.,160,0.,16.);
    //    TH2F *h2 = new TH2F("h","",60,150.,180.,160,0.,8.);
    TH2F *h2 = new TH2F("h", "", 100, 90., 180., 100, 0., 20.);
    // h2->SetXTitle("#theta_{lab} (deg)");
    // h2->SetYTitle("TKE_{lab} (MeV)");
    h2->SetXTitle("#theta_{p} [deg]");
    h2->SetYTitle("E_{p} [MeV]");
    ...
    for (Int_t iEx = 0; iEx < nEx; iEx++)
    {
        Double_t ex = iEx * estep + exmin;
        ex = (ex > exmax) ? exmax : ex;
        grEx[iEx]->SetLineColor(kGray + 1);
        if (int(ex / 10.) == int(ex) / 10.)
        {
            printf("%d %f\n", int(ex / 10.), int(ex) / 10.);
            // grEx[iEx]->SetLineColor(kBlue);
            grEx[iEx]->SetLineColor(kBlue - 7);
    ...
    for (Int_t iq = 0; iq < nQLine; iq++)
    {
        grTh[iq]->SetLineColor(kGray + 1);
        if ((iq % 5) == 0)
        {
            // grTh[iq]->SetLineColor(kRed);
            grTh[iq]->SetLineColor(kRed - 7);
