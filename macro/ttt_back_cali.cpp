...
void ttt_back_cali(TH2F *hin)
{
    ofstream myfile;
    myfile.open("test/ttt_back_para.txt");
    ...
    const double ymin = back->GetYaxis()->GetXmin();
    const double ymax = back->GetYaxis()->GetXmax();
    ...
    for (int i = 1; i <= N; i++)
    {
        ...
        if (i == 221||i==249||i==256){
            Npeak = spec->Search(h[i], 0, "", 0.35);
        }
        else{
            Npeak = spec->Search(h[i], 0, "", 0.25);
        }
