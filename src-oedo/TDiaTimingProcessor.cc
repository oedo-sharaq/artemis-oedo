...
using art::TDiaTimingProcessor;

ClassImp(TDiaTimingProcessor)

#define N_MAX_STRIP 6 // number of max diamond strips. In SHARAQ03, we used 6 strip in total (2+4=6).

TDiaTimingProcessor::TDiaTimingProcessor()
{
...
TDiaTimingProcessor::TDiaTimingProcessor(const TDiaTimingProcessor& rhs)
{
}

TDiaTimingProcessor& TDiaTimingProcessor::operator=(const TDiaTimingProcessor& rhs)
{
   if (this != &rhs) {
...
void TDiaTimingProcessor::Init(TEventCollection *col)
{
   Info("Init","(%s,%s) => %s",
...
void TDiaTimingProcessor::Process()
{
   fOutput->Clear("C");
   Int_t nHit1 = (*fInput1)->GetEntriesFast();
   Int_t nHit2 = (*fInput2)->GetEntriesFast();

   Double_t t_strip_each[nHit1], t_strip = kInvalidD, t_pad = kInvalidD;
