...
#include "TMWDCTrackingAllLRCombination.h"
...
using art::TMWDCTrackingAllLRCombination;

ClassImp(art::TMWDCTrackingAllLRCombination);

// Default constructor
TMWDCTrackingAllLRCombination::TMWDCTrackingAllLRCombination()
     ...
     fZ(NULL), fX(NULL), fY(NULL), fStopwatch(NULL)
{
   ...
   RegisterOutputCollection("OutputCollection","output",
			    ...
			    &fMWDCTrackOut, "TClonesArray", "art::TMWDCTrackingAllLRCombinationResult");
...
void TMWDCTrackingAllLRCombination::Init(TEventCollection *col)
{
...
void TMWDCTrackingAllLRCombination::GenerateMatrix()
{
...
void TMWDCTrackingAllLRCombination::Process()
{
   {/* initialization */
      fStopwatch->Start();
...
void TMWDCTrackingAllLRCombination::ProcessOne(const std::vector<std::vector<TMWDCHitData*> >& planeData)
{
   fNPlaneValid = 0;
   fCombinator->Clear();
   for (Int_t iPlane = 0; iPlane != fNPlane; ++iPlane) {
   ...
   TMWDCTrackingAllLRCombinationResult *const tr =
      static_cast<TMWDCTrackingAllLRCombinationResult*>(fMWDCTrackOut->ConstructedAt(fMWDCTrackOut->GetEntriesFast()));
...
Int_t TMWDCTrackingAllLRCombination::FindBestCombination(TMWDCTrackingAllLRCombinationResult *tr)
{
   Int_t nCombinationProcessed = 0;
   {
	 ...
	 tr->SetTrackingID(TMWDCTrackingAllLRCombinationResult::kLargeCombination);
	 return 0;
      }
   }
...
Bool_t TMWDCTrackingAllLRCombination::DataIsValid(const TMWDCHitData *planeData) const
{
   {
...
Bool_t TMWDCTrackingAllLRCombination::CombinationIsValid() const
{
   return kTRUE;
}

void TMWDCTrackingAllLRCombination::FindTrack(TMWDCTrackingAllLRCombinationResult *tr, Bool_t fillStat)
{
   ...
   std::vector<Double_t> LR(fNPlane,kInvalidD);
   std::vector<Double_t> LRAdopted(fNPlane,kInvalidD);
   std::vector<Double_t> SR(fNPlane,kInvalidD);
   std::vector<Double_t> SRAdopted(fNPlane,kInvalidD);
   ...
   for(Int_t i = 0, N = TMath::Nint(TMath::Power(2,fNPlane)); i!= N ; ++i){
     ...
     for(Int_t iPlane = 0; iPlane !=fNPlane ; ++iPlane){
        const Double_t trackedPosition_SR =
  	 fNParameter == 2 ?
  	 ...
  	 + fSin[iPlane] * (trackPrm[1] + trackPrm[3] * fZ[iPlane]);
         SR[iPlane] = pos[iPlane] - trackedPosition_SR;
       }
      if(SSR < minSSR){
         minSSR = SSR;
	 std::copy(trackPrm.begin(),trackPrm.end(),bestTrackPrm.begin());
	 std::copy(residual.begin(),residual.end(),bestResidual.begin());
	 std::copy(LR.begin(),LR.end(),LRAdopted.begin());
	 std::copy(SR.begin(),SR.end(),SRAdopted.begin());
      }
   }
   ...
   for(Int_t i=0;i!=fNPlane;++i){
	  ...
	  tr->SetLRAdopted(i,LRAdopted[i]);
	  tr->SetSRAdopted(i, SRAdopted[i]);
   }

   if (minSSR < fSSRMax) {
      tr->SetTrackingID(TMWDCTrackingAllLRCombinationResult::kGood);
	 ...
	 for (Int_t i = 0; i != fNParameter; ++i) {
      ...
      tr->SetTrackingID(TMWDCTrackingAllLRCombinationResult::kLargeSSR);
   }
}

Double_t TMWDCTrackingAllLRCombination::CalcSSR(const std::vector<Double_t> &pos,
